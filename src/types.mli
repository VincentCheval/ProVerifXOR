...
and term =
  ...
  | FAC of funsymb * mterm list

(** [mterm] is the type of terms with their multiplicity. *)
and mterm = term * int
...
(** An AC_fact_format [fact,vars] is a fact_format where the variables in [vars] are the ones
  not declared with a * (i.e. FAny). They thus must only be matched by variables. *)
type ac_fact_format = fact * binder list
...
type history =
    Rule of int * label * (fact * marking) list * fact * constraints
...
type clauses =
  | Given of marked_reduction list
  | ToGenerate of marked_reduction list * ((marked_reduction -> unit) -> unit)
...
type t_horn_state =
    { h_clauses : clauses;
      ...
      h_clauses_to_initialize_selfun : marked_reduction list;
