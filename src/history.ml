    ...
    let (hyp,concl,constra,hdescr) =
      match descr with
        RElem(preds, ({ p_type = [t1;t2] } as p)) ->
          ...
          (List.map (fun p' -> Pred(change_type_attacker p' t1, [v]), NoMarking) preds,
           Pred(p, [v;v']),
           true_constraints, Elem(preds,p))
      | RApplyFunc(f,p) ->
          
          (* Return the application rule (no variant) *)
          let rec gen_vars n =
            if n = 0 then
              []
            else
              if f.f_AC
              then (Terms.apply_symbol_margs f (Terms.var_gen_mterm (fst f.f_type))) :: gen_vars (n-1)
              else (FunApp(f, Terms.var_gen (fst f.f_type))) :: (gen_vars (n-1))
          in
          ...
          let tag = match f.f_cat with
            ...
            | _ -> Apply(f,p)
          in
          (List.map (fun h -> Pred((change_type_attacker p (Terms.get_term_type (List.hd h))), h), NoMarking) hypl,
      ...
      | RApplyProj(f,nproj,p) ->
          ...
          ([Pred((change_type_attacker p (Terms.get_term_type (List.hd hyp))),hyp),NoMarking],
           Pred((change_type_attacker p (Terms.get_term_type (List.hd concl))), concl), true_constraints,
      ...
      | RFail(left,({ p_info = AttackerBin(_,t) | AttackerGuess t; _ } as p)) ->
          ...
          let hyp = if left then [Pred(p, [x; fail]),NoMarking] else [Pred(p, [fail; x]),NoMarking] in
...
module HashFact =
  struct
    ...
    type skel_term =
      ...
      | SAC of skel_f * (skel_term * int) list
    ...
    let rec skeleton_term = function
      ...
      | FAC(f,margs) ->
          SAC(skel_f f,List.map (fun (t,k) -> skeleton_term t, k) margs)
...
let get_terms_to_unify_facts exc f1 f2 = match f1, f2 with
  | Pred(p1,args1), Pred(p2,args2) ->
      if p1 == p2
      then List.map2 (fun a b -> (a,b)) args1 args2
      else raise exc

let get_terms_to_unify_facts_phase exc f1 f2 = match f1, f2 with
  | Pred(p1,args1), Pred(p2,args2) ->
      if Terms.is_sub_predicate p1 p2
      then List.map2 (fun a b -> (a,b)) args1 args2
      else raise exc

let rec copy_fact_tree t = 
  {
    desc = copy_fact_tree_desc t.desc;
    thefact = Terms.copy_fact2 t.thefact
  }

and copy_label = function
  | ProcessRule(hl,tl) -> 
      ProcessRule(hl,List.map copy_term2 tl)
  | lbl -> lbl 

and copy_fact_tree_desc = function
  | FRemovedWithProof ft -> FRemovedWithProof (copy_fact_tree ft)
  | FRule(n,l,cons,ft_list,cons2,added) ->
      FRule(n,copy_label l,Terms.copy_constra2 cons,List.map copy_fact_tree ft_list,Terms.copy_constra2 cons2,List.map copy_fact_tree added)
  | FEquation ft -> FEquation (copy_fact_tree ft)
  | ftd -> ftd

let rec build_fact_tree_rec = function
  | Empty(f) ->
      let f' = Terms.auto_cleanup (fun () -> copy_fact2 f) in
      { desc = FEmpty; thefact = f' }, []
  | Any(n, h) ->
      let t,terms_to_unify = build_fact_tree_rec h in
      let d = get_desc "any" t n in
      begin match d.thefact with
        Pred(p, a::r) when p.p_prop land Param.pred_ANY_STRICT != 0
          ...
          let unify_arguments = List.map (fun b -> (a,b)) r in
          d.desc <- FHAny;
          t, (List.rev_append unify_arguments terms_to_unify)
      | _ -> 
          d.desc <- FHAny;
          t, terms_to_unify
      end
  ...
  | Removed(rem_count, dup_count, h) ->
      ...
      let unify_arguments = get_terms_to_unify_facts (HistoryUnifyError("HistoryRemoved")) d1.thefact d2.thefact in
      t, (List.rev_append unify_arguments terms_to_unify)
  | HEquation(n,leq,req,h) ->
     let t, terms_to_unify = build_fact_tree_rec h in
     (* Copy the facts *)
     let leq',req' = Terms.auto_cleanup_nocatch (fun () -> copy_fact2 leq, copy_fact2 req) in
     if n = -1
    then
      let unify_arguments = get_terms_to_unify_facts (HistoryUnifyError("HistoryEquation")) req' t.thefact in
      { desc = FEquation(t); thefact = leq' }, (List.rev_append unify_arguments terms_to_unify)
    else
      begin 
        let d = get_desc "equation" t n in
        let unify_arguments = get_terms_to_unify_facts (HistoryUnifyError("HistoryEquation2")) leq' d.thefact in
        d.desc <- FEquation({ desc = FEmpty; thefact = req' });
        t, (List.rev_append unify_arguments terms_to_unify)
      end
  | Rule(n,descr,hyp,concl,constra) ->
      let rhyp, rconcl, rconstra, rdescr = 
        Terms.auto_cleanup_nocatch (fun () ->
          List.map (fun (fact,mk) -> copy_fact fact, mk) hyp,
          copy_fact concl,
          copy_constra constra,
          match descr with
      ...
      in      
      {
        desc = FRule(n, rdescr, rconstra, List.map (fun (f,_) -> { desc = FEmpty; thefact = f }) rhyp, true_constraints, []);
        thefact = rconcl;
      }, []
  | Resolution(h1, n, h2) ->
      let t1,terms_to_unify1 = build_fact_tree_rec h1 in
      let t2,terms_to_unify2 = build_fact_tree_rec h2 in
      let d = get_desc "resolution" t2 n in
      let unify_arguments = get_terms_to_unify_facts (HistoryUnifyError("HistoryResolution")) t1.thefact d.thefact in
      d.desc <- t1.desc;
      t2, (List.rev_append unify_arguments (List.rev_append terms_to_unify2 terms_to_unify1))
  | TestUnifTrue(n, h2) ->
      let t2,terms_to_unify = build_fact_tree_rec h2 in
  ...
  | HLemma(id_lem,matching_list,matching_sub_list,concl_lem,h) ->
      let t1,terms_to_unify = build_fact_tree_rec h in
      ...
      let unify_args1 = 
        List.fold_left (fun acc (i, fact_lem) ->
          let fact =
            if i < 0
          ...
          let fact_lem' = match fact_lem, fact with
            ...
            | _, _ -> fact_lem
          in
          List.rev_append (get_terms_to_unify_facts_phase (HistoryUnifyError("HLemma")) fact_lem' fact) acc
        ) terms_to_unify m_l'
      in

      (* Match the subterm fact of the lemma's premise *)
      let unify_args2 = 
        List.fold_left (fun acc (t_sub,t,pos_l) ->
      ...
      let unify_args3 = List.rev_append eqs_cl' unify_args2 in
  ...
  | HCaseDistinction(concl,hypl,subst,constra,h) ->
      let tree,terms_to_unify = build_fact_tree_rec h in
      ...
      let unify_args1 = List.rev_append (get_terms_to_unify_facts (HistoryUnifyError("HVerification")) tree.thefact concl') terms_to_unify in

      (* Unify the hypotheses *)
      let (unify_args2,_) = 
        List.fold_left (fun (acc,n) f ->
          let d = get_desc "verification" tree n in
          List.rev_append (get_terms_to_unify_facts (HistoryUnifyError("HVerification")) d.thefact f) acc, n+1 
        ) (unify_args1,0) hypl'
      in

      (* Unify the equalities from the substitutions *)
      let unify_args3 = List.rev_append subst' unify_args2 in
  ...
  | HInjectivity(injectivity,h) ->
      ...
      let injectivity' = Terms.auto_cleanup_nocatch (fun () -> copy_injectivity injectivity) in


      let unify_args = match injectivity' with
        | DoubleIndex(n1,n2) ->
            let d1 = get_desc "HInjectivity" tree n1 in
            let d2 = get_desc "HInjectivity" tree n2 in
            get_terms_to_unify_facts (HistoryUnifyError("HInjectivity")) d1.thefact d2.thefact
        | SingleIndex(concl,f,n) ->
            let d = get_desc "HInjectivity" tree n in
            (get_terms_to_unify_facts (HistoryUnifyError("HInjectivity")) tree.thefact concl) @
            (get_terms_to_unify_facts (HistoryUnifyError("HInjectivity")) d.thefact f)
        | NoIndex(concl) -> 
            get_terms_to_unify_facts (HistoryUnifyError("HInjectivity")) tree.thefact concl
      in

      tree, (List.rev_append unify_args terms_to_unify)
  | HNested(n_list,nb_f_in_concl,h) ->
      ...
      let unify_args,_ = 
        List.fold_left (fun (acc,i) i_match ->
          let fact =
            if i_match < 0
          ...
          List.rev_append (get_terms_to_unify_facts (HistoryUnifyError("HNested")) fact fact_nested) acc, i+1
        ) (terms_to_unify,0) n_list
      in

      tree, unify_args

let build_fact_tree h = 
  let tree,unify_args = build_fact_tree_rec h in
  try 
    AC.Ac_unify.unify_terms (fun () ->
      copy_fact_tree tree  
    ) unify_args
  with Terms.Unify ->
    Printf.printf "Error Unify when building tree\n";
...
type recheck_t = (marked_reduction -> bool) option
...
let build_history recheck ((subgoals, orig_fact, hist_done, constra, variant):marked_reduction) =
...
module HashFactId =
  struct
    ...
    let rec skeleton_term = function
      ...
      | FAC(f,margs) ->
          SAC(skel_f f, List.map (fun (t,n) -> skeleton_term t, n) margs)
      ...
      then restwork useful_vars
        ...
        | Pred(p1,[t1;_]),Pred(p2,[t2;_]) when p1 == p2 && Param.inj_event_pred_block == p1 ->
            ...
            TermsEq.unify_modulo (fun () ->
              if List.exists (fun v -> match v.link with TLink _ -> true | _ -> false ) useful_vars'  && not (constraint_satisfiable constra)
              then raise Unify;
