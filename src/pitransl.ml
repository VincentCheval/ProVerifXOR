...
let red_rules = ref ([] : marked_reduction list)
let no_gen_var = ref []
...
let add_rule_follow (f_next:marked_reduction -> unit) hyp concl constra tags variant=
  let constra = TermsEq.remove_caught_fail constra in
  let hyp = List.map (fun f -> f,NoMarking) hyp in
  let rule = (hyp, concl, Rule (!nrule, tags, hyp, concl, constra), constra, variant) in
  f_next rule;
  incr nrule

let add_rule hyp concl constra tags variant =
  let constra = TermsEq.remove_caught_fail constra in
  let rule = (hyp, concl, Rule (!nrule, tags, hyp, concl, constra), constra, variant) in
...
let rec get_other_vars other_vars vlist = function
  ...
  | FAC(_,margs) ->
      List.iter (fun (t,_) -> get_other_vars other_vars vlist t) margs

let rec is_ground bound_vars t =
  match t with
    Var v -> List.memq v bound_vars
  | FunApp(f,l) -> List.for_all (is_ground bound_vars) l
  | FAC(_,margs) -> List.for_all (fun (t,_) -> is_ground bound_vars t) margs
...
let rec binds_vars error_message bound_vars = function
  ...
  | FAC(f,margs) ->
      if not (List.for_all (fun (t,_) -> is_ground !bound_vars t) margs) 
      then
        begin
          error_message();
          user_error ("Cannot bind variables under non-data constructors ("
...
type transl_state =
    { tr_pi_state : t_pi_state; (* [pi_state] received as input *)
      ...
      *)
      ...
      record_fun_opt : (marked_reduction -> unit) option
...
let output_rule cur_state out_fact = match cur_state.record_fun_opt with
  ...
  | Some f_rule ->
      ...
      Terms.auto_cleanup (fun _ ->
        ...
        let hyp,concl,constra2,name_params2 = 
          Terms.auto_cleanup_nocatch (fun () ->
            List.map Terms.copy_fact2 cur_state.hypothesis,
            Terms.copy_fact2 out_fact,
            Terms.copy_constra2 cur_state.constra,
            List.map Terms.copy_term2 name_params
          )
        in
        begin
          try
        ...
        if !Param.key_compromise = 2 then
          begin
            ...
            let rec copy_term3 = function
              ...
              | FAC(f,margs) ->
                  let margs' = List.map (fun (t,k) -> copy_term3 t,k) margs in
                  Terms.apply_symbol_margs (if f == session0 then session1 else f) margs'
...
let check_feasible f_next check cur_state =
  ...
  then
    ...
    try
      ...
      (* TODO RE-instate this when EQ modulo works? *)
...
let apply_AC_variant next_f cur_state f t1 t2 =
  let rules = Terms.get_all_rewrite_rules_status f in
  let rw_rules = 
    List.map (fun (_,(lhs,rhs,_)) -> match lhs with 
      [t1;t2] -> lhs, rhs
      | _ -> Parsing_helper.internal_error "Should be a binary symbol"
    ) rules 
  in
...
and transl_term next_f cur_state = function
  ...
  | FAC(f,margs) -> 
      transl_mterm_list (fun cur_state1 margs' ->
        apply_AC_unification next_f cur_state1 f margs'
      ) cur_state margs
  | FunApp(f,l) ->
...
and apply_AC_unification next_f cur_state f = function
  | [] -> failwith "Should not be empty"
  | [t,2] -> apply_AC_variant next_f cur_state f t t
  | [t1,1;t2,1] -> apply_AC_variant next_f cur_state f t1 t2
  | (t,1)::q -> 
      apply_AC_unification (fun cur_state1 tq ->
        apply_AC_variant next_f cur_state1 f t tq
      ) cur_state f q
  | (t,n)::q -> 
      apply_AC_unification (fun cur_state1 tq ->
        apply_AC_variant next_f cur_state1 f t tq
...
and transl_mterm_list next_f cur_state = function
  | [] -> next_f cur_state []
  | ((t,n)::q) ->
      transl_term (fun cur_state1 t' ->
        transl_mterm_list (fun cur_state2 q' ->
          next_f cur_state2 ((t',n)::q')
        ) cur_state1 q
      ) cur_state t
  ...
  List.iteri (fun i red_rule ->
    ...
    if Param.is_noninterf pi_state then
      ...
      (* The definition of destructors by rules
        ...
        add_rule
          (((testunif_fact (FunApp(tuple_fun, vlist)) (FunApp(tuple_fun, hyp'))),NoMarking)
           :: List.map (fun t -> att_fact phase t, NoMarking) vlist)
          (Pred(bad_pred, []))
...
let rules_for_red_guess f red_rules =
  List.iter (fun red1 ->
    List.iter (fun red2 ->
      ...
      add_rule (List.map2 (fun t1 t2 -> att_guess_fact t1 t2, NoMarking) hyp1 hyp2)
...
let transl pi_state =
  ...
  if !Param.remove_subsumed_clauses_before_display then
    begin
      ...
      List.iter (function (hyp, concl, _, _,_) as rule ->
        (* eliminate tautologies *)
        if List.exists (fun (fact,_) -> Terms.equal_facts concl fact) hyp then () else
        (* eliminate subsumed clauses *)
        let (cl,feat_v,sub_data) = Database.FeatureGenClause.generate_feature_vector_and_subsumption_data rule in
        let sel_index = Selfun.selection_fun rule in
        let additional_data = 
          if sel_index = -1
          then Database.Solved (List.for_all (fun (f,_) -> Terms.is_unselectable f) hyp)
          else 
            let mark = snd (List.nth hyp sel_index) in
            Database.Unsolved (sel_index,mark)
        
        in
        let annot_rule = (cl,feat_v,sub_data,additional_data) in
      ...
      Database.SetClause.iter (fun elt ->
        ...
        | (hyp', concl', Rule(_, tags, hyp, concl, constra), constra', variant),_,_ ->
            red_rules := (hyp', concl', Rule(!nrule, tags, hyp, concl, constra), constra', variant) :: (!red_rules);
            incr nrule
