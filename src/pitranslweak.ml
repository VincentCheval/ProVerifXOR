...
and transl_term_one_side next_f cur_state do_left = function
  ...
  | FAC _ -> internal_error "AC function symbols should not be accepted with equivalence yet."
  | FunApp(f,args) ->
...
let rec convert_to_2 = function
  ...
  | FAC _ -> internal_error "AC function symbols should not be used in equivalence yet."  
  ...
  if !Param.remove_subsumed_clauses_before_display then
    begin
      ...
      List.iter (function (hyp, concl, _, _,_) as rule ->
      	...
      	if List.exists (fun (f,_) -> Terms.equal_facts concl f) hyp then () else
