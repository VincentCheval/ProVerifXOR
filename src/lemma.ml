...
let convert_lemma_for_monoprocess lemma =
  ...
  let explore_one_side left_side evl concl_q =
    ...
    let rec check_keep_variables = function
      ...
      | FAC(_,margs) -> List.iter (fun (t,_) -> check_keep_variables t) margs
    in
...
let rec follow_vlink = function
  ...
  | FAC(f,margs) -> 
      let margs' = List.map (fun (t,k) -> follow_vlink t, k) margs in
...
let rec put_constants2 = function
  ...
  | FAC(_,margs) -> List.iter (fun (t,_) -> put_constants2 t) margs
...
let rec not_ground = function
  ...
  | FAC(_,margs) -> List.exists (fun (t,_) -> not_ground t) margs
...
let rec no_bound_name_term = function
  ...
  | FAC(_,margs) -> List.for_all (fun (t,_) -> no_bound_name_term t) margs
