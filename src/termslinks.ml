...
let rec copy_term_all_link = function
  | Var { link = TLink t } -> copy_term_all_link t
  | Var { link = VLink v } -> copy_term_all_link (Var v)
  | Var v -> Var v
  | FunApp(f,args) -> FunApp(f,List.map copy_term_all_link args)
  | FAC(f,margs) -> 
      let margs' = List.map (fun (t,k) -> copy_term_all_link t, k) margs in
...
let rec equal_terms_with_links t1 t2 = (t1 == t2) || (match (t1,t2) with
  ...
  | FAC(f,margs), FAC(f',margs') ->
    if f == f'
    then 
      let t1' = copy_term_all_link t1 in
      let t2' = copy_term_all_link t2 in
      equal_terms t1' t2'
    else false
  | _,_ -> false)
...
let rec equal_closed_terms t1 t2 = match (t1,t2) with
  ...
  | FAC(f,margs), FAC(f',margs') ->
      (f==f') &&
        let t1' = copy_term4 t1 in
        let t2' = copy_term4 t2 in 
        equal_terms t1' t2'
  | _,_ -> false
...
let rec match_terms f_next t1 t2 =
  if not (Param.get_ignore_types()) then
  if (get_term_type t1 != get_term_type t2) then
    assert false;
  match (t1,t2) with
  | (Var { link = TLink t1' }, _) -> match_terms f_next t1' t2
  | (_, Var { link = TLink t2' }) -> match_terms f_next t1 t2'
  ...
  | (Var v), t ->
   ...
   | (FunApp (f1,l1')), (FunApp (f2,l2')) ->
        ...
        match_term_list f_next l1' l2'
  | FAC _, FAC _ ->
      let t1' = copy_term4 t1 in
      let t2' = copy_term4 t2 in
      AC.Ac_unify.match_terms f_next [t1',t2']
  | _ -> raise Unify

and match_term_list f_next l1 l2 = match l1,l2 with
  | [], [] -> f_next ()
  | [], _ | _, [] -> Parsing_helper.internal_error "Bad size list"
  | t1::q1, t2::q2 -> 
      match_terms (fun () ->
        match_term_list f_next q1 q2
      ) t1 t2
...
let rec get_vars vars = function
  ...
  | FAC(_,margs) -> List.iter (fun (t,_) -> get_vars vars t) margs
