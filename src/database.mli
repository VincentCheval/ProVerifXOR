...
val remove_ground_public_terms : marked_reduction -> marked_reduction
...
module type ClauseSig =
  sig
    ...
    (**
      ...
      In the case of [ordered_reduction], [match_facts f_next comply_with_marking hfact1 hhfact2] it will also check that
      the ordering function of [hfact1] subsumes the ordering function of [hfact2]
    *)
    val match_facts : (unit -> 'a) -> bool -> hyp_fact -> hyp_fact -> 'a

    (** Same as [match_facts] but all variables of [hyp1] should have already been linked (more efficient). *)
...
val record_from_rule : marked_reduction -> unit
...
module type SetSig =
  sig
    ...
    (* [deactivate_implied_by set annot_cl] deactivates the clauses from [set]
...
module DB : DBSig with type clause = marked_reduction and type queue = QueueClause.t and type set = SetClause.t and type subsumption_data = SubClause.subsumption_data
module DBOrd : DBSig with type clause = ordered_reduction and type queue = QueueOrdClause.t and type set = SetOrdClause.t and type subsumption_data = SubOrdClause.subsumption_data
