...
let elim_att_guess_xx (next_stage:marked_reduction -> 'a) (repeat_next_stage:marked_reduction -> 'a) ((hyp, concl, hist, constra, variant):marked_reduction) : 'a =
  let redo_all_optim = ref false in
  let hist' = ref hist in
  let rec f n = function
      [] -> []
    | (Pred({ p_info = AttackerGuess _}, [Var v1; Var v2]),NoMarking) :: l when v1 == v2 ->
        redo_all_optim := true;
        hist' := Resolution(List.assq (Param.get_type v1.btype) (!attrulenum), n, !hist');
        (Pred(Param.get_pred (Attacker(!max_used_phase, v1.btype)), [Var v1]),NoMarking) :: (f (n+1) l)
    | fact :: l -> fact :: (f (n+1) l)
  in
  let hyp' = f 0 hyp in
  let r' = (hyp', concl, !hist', constra,variant) in
...
let remove_equiv_events next_stage ((hyp,concl,hist,constra,variant) as r:marked_reduction) =
  if !weaksecret_mode && Terms.is_bad concl && List.for_all (function (Pred(p,_),_) -> Param.event2_pred_block == p) hyp
  then next_stage ([],concl,hist,constra,variant)
  else next_stage r

let is_standard_clause ((_,_,h,_,_) as r:marked_reduction) =
...
let simplify next_stage (repeat_next_stage:marked_reduction-> unit) ((hyp, concl, hist, constra,variant) as r:marked_reduction) =
  ...
  then
    ...
    let rec find_att x = function
        [] -> false
      | (Pred(p', [t]),_) :: _ when p'.p_prop land Param.pred_ATTACKER != 0 && Terms.equal_terms t x -> true
      | _ :: l -> find_att x l
    in
    let rec find_right x y = function
        [] -> None
      | (Pred(p', [t1; t2]),_) :: _ when p'.p_prop land Param.pred_ELIMVAR != 0 && Terms.equal_terms t1 x && not (Terms.equal_terms t2 y) -> Some t2
      | _ :: l -> find_right x y l
    in
    let rec find_left x y = function
        [] -> None
      | (Pred(p', [t1; t2]),_) :: _ when p'.p_prop land Param.pred_ELIMVAR != 0 && Terms.equal_terms t2 x && not (Terms.equal_terms t1 y) -> Some t1
      | _ :: l -> find_left x y l
    in
    ...
    let rec is_public = function
      ...
      | FAC(f,margs) ->
          (not f.f_private) && (List.for_all (fun (t,_) -> is_public t) margs)
    in
    ...
    let rec inst = function
      ...
      | (h::qr) ->
          ...
          | Pred(p, [t1; t2]),_ when (Terms.unblock_predicate p).p_prop land Param.pred_ELIMVAR != 0 ->
              begin
    ...
    try
	  ...
	  (* A variable has been linked *)
          let hyp' = List.map (fun (fact,mk) -> Terms.copy_fact2 fact, mk) hyp in
          let concl' = Terms.copy_fact2 concl in
          let constra' = Terms.copy_constra2 constra in
          Terms.cleanup();
          repeat_next_stage (hyp', concl', hist, constra',variant)
        end
      else
	(* Nothing has changed *)
...
let selfun ((hyp, concl, hist, constra, _) as r:marked_reduction) =
  if not ((!weaksecret_mode) && (Terms.is_bad concl) && (hyp != []) && List.exists (function (Pred(p,_),_) -> Param.event2_pred_block != p) hyp) then -1
  else
