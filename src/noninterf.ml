...
let rec contains_bound_name fl = function
  ...
  | FunApp(f,l) -> 
      ...
      (
        ...
        (List.exists (contains_bound_name fl) l)
      )
  | FAC(f,margs) -> 
      (not (List.memq f fl)) &&
      (
        ...
        (List.exists (fun (t,_) -> contains_bound_name fl t) margs)
      )
...
let swap_with_copy next_stage hypbefore hypafter concl hist constra variant l1 l2 =
  assert (!Terms.current_bound_vars == []);
  let l1', l2' = swap [] [] l1 l2 in
  let hypbefore' = List.map (fun (f,mk) -> Terms.copy_fact3 f, mk) hypbefore in
  let hypafter' = List.map (fun (f,mk) -> Terms.copy_fact3 f, mk)  hypafter in
  let concl' = Terms.copy_fact3 concl in
  let constra' = Terms.copy_constra3 constra in
  Terms.cleanup();
  next_stage hypbefore' hypafter' concl' hist constra' variant l1' l2'
...
let dec_out_rule next_stage hypbefore hypafter concl hist constra variant t1simp t2simp =
  match (t1simp,t2simp) with
    [],[] -> ()
  | [t1s],[t2s] ->
      let hypbefore' = (Pred(Param.get_pred (TestUnifP(Terms.get_term_type t1s)), [t1s; t2s]),NoMarking) :: hypbefore in
      next_stage hypbefore' hypafter concl hist constra variant
  | _ ->
      ...
      let hypbefore' = (Pred(Param.get_pred (TestUnifP(Param.bitstring_type)),
			     [ FunApp(tuple_fun, t1simp);
			       FunApp(tuple_fun, t2simp)]),NoMarking) :: hypbefore
      in
      next_stage hypbefore' hypafter concl hist constra variant

let check_any_x = function
    Pred(chann,[Var v]),_ -> chann.p_prop land Param.pred_ANY != 0
  | _ -> false

let secr_in_sets next_stage hypbefore hypafter concl hist constra variant t1 t2 =
  ...
  let rec unify_secrets todo = function
      [] ->
        ...
        current_bound_vars := [];
        let hypbefore' = List.map (fun (f,mk) -> Terms.copy_fact2 f, mk)  hypbefore in
        let hypafter' = List.map (fun (f,mk) -> Terms.copy_fact2 f, mk) hypafter in
...
let check_testunif_true next_stage hypbefore hypafter concl hist constra variant l1 l2 =
  if (List.for_all check_any_x hypbefore)
    ...
    begin
     ...
     secr_in_sets next_stage hypbefore hypafter concl hist' constra variant l1 l2
    end
   else
     (* Keep the rule *)
     dec_out_rule next_stage hypbefore hypafter concl hist constra variant l1 l2
...
let inst_elim next_stage (repeat_next_stage:marked_reduction -> unit) hypbefore hypafter concl hist constra variant l1 l2 =
   if not (Terms.is_bad concl) then
     next_stage hypbefore hypafter concl hist constra variant l1 l2
   else
   let redo_all_optim = ref false in
   let contains_att_x x = (List.exists (fun (f,_) -> is_att_x x f) hypbefore)
   || (List.exists (fun (f,_) -> is_att_x x f) hypafter) in
   let is_unselectable_or_removed f =
	...
	there will remain only unselectable facts. *)
     ...
     | _ -> false) ||
       (is_unselectable f)
   in
   let has_selectable () = not ((List.for_all (fun (f,_) -> is_unselectable_or_removed f) hypbefore)
      && (List.for_all (fun (f,_) -> is_unselectable_or_removed f)  hypafter)) in
   let redo_optim() =
       let hypbefore' = List.map (fun (f,mk) -> Terms.copy_fact2 f,mk) hypbefore in
       let hypafter' = List.map (fun (f,mk) -> Terms.copy_fact2 f,mk)  hypafter in
       let concl' = Terms.copy_fact2 concl in
       ...
       Terms.cleanup();
       dec_out_rule (fun hypbefore hypafter concl hist constra variant ->
	 repeat_next_stage ((List.rev_append hypbefore hypafter), concl, hist, constra,variant))
	 hypbefore' hypafter' concl' hist constra' variant l1' l2'
   in
   assert (!Terms.current_bound_vars == []);
...
let check_sets next_stage hypbefore hypafter concl hist constra variant l1 l2 =
...
let simplify next_stage repeat_next_stage ((hyp, concl, hist, constra, variant) as r:marked_reduction) =
  if (not (!non_interference)) then
    next_stage r
  else
    let rec simplify_testunif hypbefore hypafter concl hist constra variant =
      match hypafter with
	    | [] -> next_stage ((List.rev hypbefore), concl, hist, constra, variant)
      |	(a::l) ->
	  match a with
	    Pred({p_info = TestUnifP _ },[t1;t2]),_ ->
	(* In the intermediate simplification steps, the rule is in format
	   ...
	   dec_out_rule rebuilds the standard rule format.
          ...
          (swap_with_copy 
            (inst_elim 
              (check_sets 
                (check_testunif_true simplify_testunif)
              ) repeat_next_stage
            ) hypbefore l concl hist constra variant
          ) t1 t2
	  | _ -> simplify_testunif (a::hypbefore) l concl hist constra variant
    in
    simplify_testunif [] hyp concl hist constra variant
...
let selfun ((hyp, concl, _, _,_) as r:marked_reduction) =
