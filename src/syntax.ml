...
let rules = ref ([]:marked_reduction list)
...
let rec check_one_hyp (hyp_accu,constra_accu) env (fact, ext) =
  ...
  | PSimpleFact(p,l) ->
      ...
      ((Pred(p',l'),NoMarking)::hyp_accu, constra_accu)
...
let rec check_red = function
  ...
  | (Equiv(i,c,select)::l) ->
      ...
      check_red l;
      let env = Hashtbl.create 7 in
      let hyp = List.map (fun e -> let fact = check_simple_fact env e in fact, NoMarking) i in
      let concl = check_simple_fact env c in
      let requiv =
	      incr rule_counter;
	      (hyp, concl, Rule(!rule_counter, LblEquiv, hyp, concl, Terms.true_constraints), Terms.true_constraints, Protocol)
      in
      add_equiv (List.map (fun (f,_) -> f) hyp, concl, !rule_counter);
      rules :=
      (List.map (fun (h,_) ->
	      incr rule_counter;
        ...
        in
	      ([concl,NoMarking], h, Rule(!rule_counter, LblEquiv, [concl,NoMarking], h, Terms.true_constraints), Terms.true_constraints, Protocol)
        ) hyp)
      @ (requiv :: (!rules))
