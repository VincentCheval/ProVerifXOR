...
let match_modulo next_f t1 t2 =
  if TermsEq.hasEquations() 
  then TermsEq.unify_modulo next_f t1 t2
  else Termslinks.match_terms next_f t1 t2

let match_modulo_list next_f l1 l2 =
  if TermsEq.hasEquations() 
  then TermsEq.unify_modulo_list next_f l1 l2
  else Termslinks.match_term_list next_f l1 l2
...
let rec close_term = function
  ...
  | FAC(f,margs) -> List.iter (fun (t,_) -> close_term t) margs
...
let rec close_term_tmp = function
  ...
  | FAC(_,margs) -> List.iter (fun (t,_) -> close_term_tmp t) margs
...
let rec copy_closed = function
  ...
  | FAC(f,margs) -> 
      let margs' = List.map (fun (t,k) -> copy_closed t, k) margs in
...
let rec copy_closed_remove_syntactic = function
  ...
  | FAC(f,margs) -> 
      let margs' = List.map (fun (t,k) -> copy_closed_remove_syntactic t, k) margs in
      Terms.apply_symbol_margs f margs'
  | Var v ->
      match v.link with
...
let rec rev_name_subst = function
  ...
  | FAC(f,margs) -> 
      let f = non_syntactic f in
      let margs' = List.map (fun (t,k) -> rev_name_subst t, k) margs in
      Terms.apply_symbol_margs f margs'
...
let rec close_term_collect_links links = function
  ...
  | FAC(_,margs) -> List.iter (fun (t,_) -> close_term_collect_links links t) margs
...
let rec check_delayed_names_t = function
  ...
  | FAC(f,margs) -> 
      let margs' = List.map (fun (t,k) -> check_delayed_names_t t, k) margs in
      Terms.apply_symbol_margs f margs'
...
let rec has_name_t = function
  ...
  | FAC(_,margs) -> List.exists (fun (t,_) -> has_name_t t) margs
  | Var _ -> false
