...
let auth_ordered_rule_of_ordered_rule ({ rule = (hypl, concl, hist, constra,_); _ } as ord_rule) = match ord_rule.order_data with
  | None ->
      let hypl' = List.map (fun (hyp,_) -> (hyp,([],0))) hypl in
      ((hypl',concl,hist,constra):auth_ordered_reduction)
  | Some ord_data ->
      let hypl' = List.map2 (fun (hyp,_) data -> hyp,data) hypl ord_data in
      (hypl',concl,hist,constra)

let auth_ordered_rule_of_marked_rule ((hypl, concl, hist, constra,_):marked_reduction) =
  let hypl' = List.map (fun (hyp,_) -> (hyp,([],0))) hypl in
  ((hypl',concl,hist,constra):auth_ordered_reduction)

let ordered_rule_of_auth_ordered ((hypl,concl,hist,constra):auth_ordered_reduction) =
  let (hypl',order_data) =
    ...
    begin
      try
        ...
        then 
          begin 
            let second_result = 
              let (hyp,concl,hist,constra,_) = cl.rule in
...
let rec specvar_to_var = function
  ...
  | FAC(f,margs) -> 
      let margs' = List.map (fun (t,k) -> specvar_to_var t, k) margs in
...
let get_clauses_for_preds () =
  match !clauses_for_preds with
    ...
    | None ->
        ...
        List.iter (function
          | (_,_,Rule(_,(Apply _ | Init | PhaseChange | TblPhaseChange), _,_,_), _,_) as cl -> clauses := cl :: (!clauses)
          | _ -> ()
        ) (!init_clauses);
...
*)
let compare_two_clauses lemmas ((n1,ev1),(hyp1,concl1,hist1,constra1,variant1),occ_concl1,inj_data1) ((n2,ev2),(hyp2,concl2,hist2,constra2,variant2),occ_concl2,inj_data2) =
  try
    AC.Ac_unify.unify_facts (fun () ->
      Terms.auto_cleanup (fun () ->
        ...
        let rule_combined =
          ...
          in
          let hyp = [Pred(simple_pred1,inj_data1.arguments),NoMarking;Pred(simple_pred2,inj_data2.arguments),NoMarking]
...
let rec match_predicate_block_list pred_l hypl = match pred_l with
  | [] -> ()
  | (Pred(p,args),_)::q when p.p_prop land Param.pred_BLOCKING != 0 ->
...
let match_unblock_predicates_same_ord_fun lemmas g_pred_unblock g_constra pred_unblock_cl pred_block_cl =
  ...
  Terms.auto_cleanup (fun () ->
    ...
    let g_pred_marked_unblock' = List.map (fun f -> f,NoMarking) g_pred_unblock' in
    ...
    Display.auto_cleanup_display (fun () ->
      ...
      try
        let (hyp, concl, hist, constra,_) =
          List.find (function
            | (hyp, _, _, c,_) when Terms.is_true_constraints c ->
                begin
                  try
...
let negate_predicate_constra lemmas ind_lemmas (hypl,concl,hist,constra) g_constra_to_negate =
  ...
  in
  let marked_hypl = List.map (fun f -> f,NoMarking) hypl in
  ...
  then None
    ...
    let hist' =
      ...
      then hist
        ...
        Terms.auto_cleanup (fun () ->
          let concl' = Terms.copy_fact2 concl in
          let hypl' = List.map (fun (f,mk) -> Terms.copy_fact2 f,mk) hypl' in
          let constra' = Terms.copy_constra2 g_constra_to_add1 in
          HCaseDistinction(concl',List.map (fun (f,_) -> Terms.copy_fact2 f) hypl',[],constra',hist)
        )
    in
  ...
  let rule_nested =
    ...
    let (fresh_fact_for_hyp, args_for_hyp) =
      List.fold_right (fun (Pred(p,_),_) (acc_f,acc_a) ->
        ...
        (Pred(p,args),NoMarking)::acc_f, args@acc_a
      ) fact_for_hyp ([],[])
    in
    let concl_rule = match new_concl with
      | Pred(p,_) -> Pred(p,args_old_concl_pred@args_for_hyp)
    in
    let hyp_rule = (Pred(old_concl_pred,args_old_concl_pred),NoMarking)::fresh_fact_for_hyp in
...
let rec clause_match_realquery restwork strong_verif lemmas ind_lemmas initial_nb_premise (((hyp,concl,_,constra) as clause):auth_ordered_reduction) = function
  | Before(evl_premise,concl_q) ->
      ...
      let (_,premise,_,_,_) = event_list_to_rule evl_premise_no_constra in
...
let verify_non_inj_query strong_verif display_when_true nested list_started lemmas ind_lemmas (Before(evl,_) as query) =
  ...
  let display_attack cl =
    let recheck_fun cl =
      success_clauses := [];
      let res = check_non_inj_clauses None strong_verif query lemmas [] initial_nb_premise [auth_ordered_rule_of_marked_rule cl] in
      success_clauses := [];
      res = True
    in
