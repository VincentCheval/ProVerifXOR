...
let rec term_evaluation = function
  ...
  | FAC(f,margs) -> 
      let margs' = List.map (fun (t,k) -> term_evaluation t, k) margs in
      if List.exists (fun (t,_) -> Reduction_helper.is_fail t) margs' then
        Terms.get_fail_term (snd f.f_type)
      else
...
let rec term_evaluation_letfilter = function
  ...
  | FAC(f,margs) ->
      let margs' = List.map (fun (t,k) -> term_evaluation_letfilter t, k) margs in
      Terms.apply_symbol_margs f margs'
...
let rec can_be_built public t =
  ...
  with Not_found ->
    ...
    | FAC({f_private = false; _} as f,margs) ->
        begin match can_be_built_mterm_list public margs with
        | None -> None
        | Some margs' -> Some(Terms.apply_symbol_margs f margs')
        end
    | _ ->
        None
...
and can_be_built_mterm_list public = function
  | [] -> Some []
  | (hd,mul)::tail ->
      match can_be_built public hd with
        | None -> None
        | Some ca ->
            match can_be_built_mterm_list public tail with
              | None -> None
              | Some catail -> Some ((ca,mul)::catail)
