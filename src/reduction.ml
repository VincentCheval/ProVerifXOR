          ...
          auto_cleanup_red (fun () ->
            ...
            with Unify ->
              ...
              let false_required_for_else_branch =
                List.map (fun { rule = (hyp, _, _, _,_); _ } ->
                  ...
                  let filtered_hyp =
                    List.filter_map (function
                        Pred(p,l),_ when p.p_prop land Param.pred_BLOCKING != 0 ->
                          (* When [Pred(p,l)] unifies with an element of [prev_state.hyp_not_matched],
                             ...
                             false as well. *)
                          if List.for_all (function (_, Pred(p',l')) when p == p' ->
                            begin
                            try
...
let rec has_sid_term sid = function
  ...
  | FAC(_,margs) -> List.exists (fun (t,_) -> has_sid_term sid t) margs
    ...
    match_query_event_list (fun event_table' ->
      ...
      TermsEq.close_constraints_eq_synt (fun constra' ->
          if facts = [] then
            ...
            let (facts,constra) = 
              Terms.auto_cleanup (fun () ->
                List.map (fun f -> Terms.copy_fact2 (TermsEq.remove_syntactic_fact f), NoMarking) facts,
