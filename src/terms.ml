...
let rec compare_term t1 t2 = match t1, t2 with
  | (FunApp(f1,_) | FAC(f1,_)), (FunApp(f2,_) | FAC(f2,_)) ->
      ...
      | 0 -> 
          ...
          | _ -> internal_error "[compare_term] The symbols being equal, the term should have the same type."
          end
      | c -> c
      end
  | (FunApp _ | FAC _) , _ -> -1
...
let apply_symbol_margs f margs = 
  ...
  then
    ...
    in
    FAC(f,margs)
  else Parsing_helper.internal_error "Expected AC Function symbol [apply_symbol_margs]"

let string_of_fixed_of_renamable = function
  | Fixed s -> s
  | Renamable r -> r.name ^ "_" ^ (string_of_int r.idx)

let string_of_funsymb f = string_of_fixed_of_renamable f.f_name
...
let get_term_type = function
    Var b -> b.btype
  | FunApp(f,_) | FAC(f,_) -> snd f.f_type
...
let accumulator_variable_id = ref 0

let new_var ?orig ?(may_fail=false) s t =
  incr accumulator_variable_id;
  let s0 =
    if may_fail then
      ...
      s
  in
  { vname = new_id ?orig s0; v_id = !accumulator_variable_id; unfailing = may_fail; btype = t; link = NoLink }
...
let var_gen_mterm tl = List.map (fun ty -> new_var_def_term ty, 1) tl
...
let rec occurs_var v = function
    Var v' -> v == v'
  | FunApp(_,l) -> List.exists (occurs_var v) l
  | FAC(_,margs) -> List.exists (fun (t,_) -> occurs_var v t) margs
...
let rec occurs_vars_all bl = function
  ...
  | FAC(_,margs) -> List.for_all (fun (t,_) -> occurs_vars_all bl t) margs

let occurs_vars_all_fact bl = function
  | Pred(_,args) -> List.for_all (occurs_vars_all bl) args
...
let rec occurs_vars_follows v = function
  ...
  | FAC(_,margs) -> List.exists (fun (t,_) -> occurs_vars_follows v t) margs
...
let rec occurs_f f = function
  ...
  | FAC(f',margs) ->
      (f == f') || (List.exists (fun (t,_) -> occurs_f f t) margs)
...
let rec is_ground_public = function
  ...
  | FAC(f,margs) when f.f_private = false -> List.for_all (fun (t,_) -> is_ground_public t) margs
  | _ -> false
...
let rec is_public = function
  ...
  | FAC(f,margs) when f.f_private = false -> List.for_all (fun (t,_) -> is_public t) margs
  | _ -> false
...
| FAC(_,margs) -> List.exists (fun (t,_) -> contains_destructor t) margs
| _ -> false
  ...
  if not v.unfailing then
    begin
      ...
      |	NoLink ->
        ...
        assert false
      |	FLink _ | PGLink _ | SLink _ | ETLink _ | Marked | ILink _ -> ()
    end;
...
let auto_cleanup_noreset f = 
  ...
  let tmp_current_bound_vars = ref [] in 
  ...
  let rec reset_link l = 
    ...
    then ()
      ...
      | v::q -> 
          ...
          reset_link q
      | _ ->
          Printf.printf "\ntmp_bound_vars length = %d\n" (List.length tmp_bound_vars); 
          Printf.printf "Input current_bound_vars length = %d\n\n" (List.length !tmp_current_bound_vars); 
          failwith "[auto_cleanup_noreset] The empty list should have been caught before."
      end
  in

  try
    let r = f () in
    tmp_current_bound_vars := !current_bound_vars;
    if List.length !tmp_current_bound_vars < List.length tmp_bound_vars
    then assert(false);
...
let auto_cleanup_nocatch f = auto_cleanup f
  (* let tmp_bound_vars = !current_bound_vars in
  current_bound_vars := [];
  let r = f () in
  List.iter (fun v -> v.link <- NoLink) (!current_bound_vars);
...
let rec generalize_vars_not_in vlist = function
  ...
  | FAC(f,margs) ->
      let margs' = List.map (fun (t,k) -> generalize_vars_not_in vlist t, k) margs in
...
let rec generalize_vars_in vlist = function
  ...
  | FAC(f,margs) ->
      let margs' = List.map (fun (t,k) -> generalize_vars_in vlist t, k) margs in
      apply_symbol_margs f margs'
...
let rec copy_term term = match term with
  ...
  | FAC(f,margs) ->
      let l' = mapq_list copy_mterm margs in
      if margs == l'
      then term
      else apply_symbol_margs f l'
  | Var v ->
      match v.link with
        NoLink ->
          let r = copy_var v in
...
and copy_mterm ((t,n) as mt)= 
  let t' = copy_term t in
  if t == t' then mt else (t',n)
...
let copy_marked_fact ((fact,mk) as mf) = match fact with
  | Pred(chann, args) ->
      let args' = mapq_list copy_term args in
      if args == args' then mf else Pred(chann, args'), mk
...
let rec ensure_not_fail = function
  | Var { link = TLink t; _ } -> ensure_not_fail t
  | Var v ->
      if v.unfailing 
      then 
        (* The variable may fail so we link it to a normal variable *)
        let v' = new_var_def_term v.btype in
        link v (TLink v')
...
let rec occur_check v t =
  ...
  | FAC(_,l) -> List.iter (fun (t,_) -> occur_check v t) l
...
let rec unify t1 t2 =
  ...
  | FAC _, _ | _, FAC _ -> failwith "Syntactic unification should not be applied on term with AC symbols."
...
let rec copy_term2 = function
  ...
  | FAC(f,l) as t ->
      let l' = mapq_list copy_mterm2 l in
      if l == l' then t else apply_symbol_margs f l'
  | Var v ->
      match v.link with
	| NoLink ->
	    let r = copy_var v in
...
let copy_marked_fact2 ((fact,mk) as mf) = match fact with
  | Pred(chann, t) ->
      let t' = mapq_list copy_term2 t in
      if t == t' then mf else Pred(chann, t'), mk
   ...
   match (t1,t2) with
     (Var v), t ->
       begin
         ...
         | TLink2 _ -> internal_error "Bad link in match_terms TLink2"
         | Marked -> internal_error  "Bad link in match_terms Marked"
         | ILink _ -> internal_error "Bad link in match_terms ILink"
         | VLink _ -> internal_error "Bad link in match_terms VLink"
...
let rec term_size = function
    Var _ -> 0
  | FunApp(_, l) -> 1 + term_list_size l
  | FAC(_,margs) -> 
      List.fold_left (fun acc_s (t,k) -> 
       let s = term_size t in
       (s+1)*k + acc_s
      ) (-1) margs
...
let rec replace_f_var vl term = match term with
  ...
  | FAC(f2,ml) ->
      let l' =  mapq_list (replace_f_var_mterm vl) ml in
      if ml == l' then term else apply_symbol_margs f2 l'

and replace_f_var_mterm vl ((t,n) as mt) = 
  let t' = replace_f_var vl t in
  if t == t' then mt else (t',n)
...
let rec follow_link var_case = function
  ...
  | FAC(f,l) -> apply_symbol_margs f (List.map (fun (t,n) -> follow_link var_case t,n) l)
...
let rec replace_name f t t' = match t' with
  ...
  | FunApp(f',l') ->
      ...
      if l' == l'' then t' else FunApp(f', l'')
  | FAC(f',margs) ->
      let l'' = mapq_list (fun (t',n) -> replace_name f t t',n) margs in
      if margs == l'' then t' else apply_symbol_margs f' l''

(* Return true when the term contains a variable *)

let rec has_vars = function
  | Var _ -> true
  | FunApp(_, l) -> List.exists has_vars l
  | FAC(_,margs) -> List.exists (fun (t,_) -> has_vars t) margs

(* List of variables *)

let rec get_vars vlist = function
  | Var v -> if not (List.memq v (!vlist)) then vlist := v :: (!vlist)
  | FunApp(_,l) -> List.iter (get_vars vlist) l
  | FAC(_,margs) -> List.iter (fun (t,_) -> get_vars vlist t) margs
...
let rec get_unlinked_vars_acc vlist = function
  ...
  | Var v -> if not (List.memq v (!vlist)) then vlist := v :: (!vlist)
  | FunApp(_,l) -> List.iter (get_unlinked_vars_acc vlist) l
  | FAC(_,margs) -> List.iter (fun (t,_) -> get_unlinked_vars_acc vlist t) margs
...
let rec mark_variables = function
  ...
  | FAC(_,margs) -> List.iter (fun (t,_) -> mark_variables t) margs
...
let rec copy_term3 = function
  | FunApp(f,l) as t ->
      ...
      if l == l' then t else FunApp(f, l')
  | FAC(f,l) as t ->
      let l' =  mapq_list copy_mterm3 l in
      if l == l' then t else apply_symbol_margs f l'
...
and copy_mterm3 ((t,n) as mt) = 
  let t' = copy_term3 t in
  if t == t' then mt else (t',n)
...
let rec copy_term4 = function
  | FunApp(f,l) as t ->
      ...
      if l == l' then t else FunApp(f, l')
  | FAC(f,l) as t ->
    let l' = mapq_list copy_mterm4 l in
    if l == l' then t else apply_symbol_margs f l'
  | Var v as t-> match v.link with
      | NoLink -> t
...
and copy_mterm4 ((t,n) as mt) = 
  let t' = copy_term4 t in
  if t == t' then mt else (t',n)
...
let copy_marked_fact4 ((fact,mk) as mf) = match fact with
  | Pred(chann, t) ->
      let t' = mapq_list copy_term4 t in
      if t == t' then mf else Pred(chann, t'), mk
...
let fun_app f = function
  ...
  | FAC(f',[t1,1;t2,1]) when f == f' -> [t1;t2]
  | _ -> raise Not_found
...
let get_session_id_from_occurrence = function
  | Var _ | FAC _ -> Parsing_helper.internal_error "The term should be an occurrence name, hence a non-AC function application"
  | FunApp(n,args) ->
      try
...
let rec put_constants = function
  ...
  | FAC(f,margs) -> List.iter (fun (t,_) -> put_constants t) margs
...
let rec has_choice = function
  ...
  | FAC(_,margs) -> List.exists (fun (t,_) -> has_choice t) margs
...
let rec choice_in_term n t = match t with
    (Var _) -> t
  ...
  | FunApp(f, l) ->
      ...
      if l == l' then t else FunApp(f,l')
  | FAC(f,margs) ->
      let margs' = mapq_list (choice_in_mterms n) margs in
      if margs == margs' then t else apply_symbol_margs f margs'

and choice_in_mterms n ((t,k) as mt) = 
  let t' = choice_in_term n t in
  if t == t' then mt else (t',k)
...
let zero_xor_term = FunApp(zero_xor_fun,[])

let xor_fun =
  let f = 
    { f_name = Fixed "xor";
      f_type = [Param.bitstring_type;Param.bitstring_type], Param.bitstring_type;
      f_cat = Eq [];
      f_initial_cat = Eq [];
      f_AC = true;
      f_private = false;
  ...
  let y = new_var_def_term Param.bitstring_type in 
  let z = new_var_def_term Param.bitstring_type in 
  let xy = apply_symbol f [x;y] in
  let yz = apply_symbol f [y;z] in
  let xz = apply_symbol f [x;z] in
...
let red_rules_constructor f =
  ...
  (vars1, apply_symbol f vars1,true_constraints) ::
...
let projection_fun = Param.memo (fun (f_symb,i) ->
  ...
  | Tuple ->
	...
	[
          ...
          [x], fail', (constraints_of_neq x (apply_symbol f_symb gen_var_list));
          [fail], fail', true_constraints
	]
	in
...
let rec clauses_for_function clauses_for_red_rules f =
  ...
  then
    ...
    | Eq red_rules ->
        let vars1 = var_gen (fst f.f_type) in
        let r_term = apply_symbol f vars1 in
        let red_rules = (vars1, r_term,true_constraints) :: (List.map (fun (lhs,rhs) -> (lhs, rhs, true_constraints)) red_rules) in
	...
	let vars1 = var_gen (fst f.f_type) in
	clauses_for_red_rules f [(vars1, apply_symbol f vars1 ,true_constraints)];
...
let rec term_never_fail = function
  ...
  | FAC(_,margs) -> List.for_all (fun (t,_) -> term_never_fail t) margs
...
let generate_destructor_with_side_cond prev_args lht_list rht ext =
  ...
  let rec get_may_fail_vars varsl term = match term with
    ...
    | FAC(_,margs) -> List.iter (fun (t,_) -> get_may_fail_vars varsl t) margs
  in
