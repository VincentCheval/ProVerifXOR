...
(** [internal_error] should be given as argument [__POS__] and an error message. *)
...
sig
  ...
  (** [for_all2_loose f [t1;...;tn] [u1;...;tk]] returns [true] iff [n = k] and [f t1 u1 = true],
      ... [f tn un = true].*)
  val for_all2_loose : ('a -> 'b -> bool) -> 'a list -> 'b list -> bool
      
  (** [filter_rev f l] equivalent to [rev (filter f l)] but more efficient. *)
  val filter_rev : ('a -> bool) -> 'a list -> 'a list

  (** [filterq f l] is the same as [filter f l] but the output is physically equals to [l]
      if no element have been filtered,
    *)
    val filterq : ('a -> bool) -> 'a list -> 'a list
  ...
  (** [string_of f b_symb e_symb sep [t1;...;tn]] returns the string corresponding to:
      [b_symb ^ f t1 ^ ... ^ f tn ^ e_symb] *)
  val string_of : ('a -> string) -> string -> string -> string -> 'a list -> string

  (** [display f b_symb e_symb sep l] is similar to [string_of] but prints it on [stdout]. *)
  val display : ('a -> unit) -> string -> string -> string -> 'a list -> unit
end

(** [lexicographic_compare comp l1 l2] compares in lexicographic order [l1] and [l2]. 
    Returns [-1] when [l1] is smaller than [l2], [0] when they are equals and [1] otherwise. 
    When [comp x1 x2 = i] when [i<0], [i=0] or [i>1] then the elements [x1] is considered smaller,
    equal to, or greater then [x2] respectively. *)
val lexicographic_compare : ('a -> 'a -> int) -> 'a list -> 'a list -> int