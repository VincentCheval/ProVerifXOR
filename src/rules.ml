...
let debug_check_ordered_rule msg ord_rule = match ord_rule.order_data with
  | None -> ()
  | Some ord_data ->
      let (hypl,concl,_,_,_) = ord_rule.rule in
...
let implies_ordering_function_bool ord_fun1 ord_fun2 =
  try Database.implies_ordering_function ord_fun1 ord_fun2; true with Unify -> false
...
let saturated_reduction_of_reduction ((hypl, concl, _, _,_) as rule) =

  let (application_result, strict_order_to_compute) =
    List.fold_right (fun (fact,_) (accu_ord,accu_strict) ->
...
let ordered_rule_of_analysed_history_op ana_hist_op ((_,_,hist,_,_) as rule) =
...
let analyse_history last_analysed_history_op ((_,_,hist,_,_):marked_reduction) =
...
let analysed_history_op_of_ordered_rule = function
  ...
  | { order_data = Some ord_data; rule = (_,_,hist,_,_) } ->
...
let rec get_vars_rep vlist = function
  ...
  | FAC(_,margs) -> List.iter (fun (t,_) -> get_vars_rep vlist t) margs 
...
let rec cancel_induction restwork vl hyp hist n = match hyp with
  | [] -> restwork hyp hist
  | (pred,_)::q_hyp  when List.exists (fun v -> occurs_var_fact v pred) vl ->
      cancel_induction (fun hyp1 hist1 ->
        restwork hyp1 (HMaxHyp(n,hist1))
      ) vl q_hyp hist (n+1)
  | (pred,_) as mk_fact::q_hyp ->
      cancel_induction (fun hyp1 hist1 ->
        restwork (mk_fact::hyp1) hist1
      ) vl q_hyp hist (n+1)

let limit_induction f_next f_next_again ((hyp,concl,hist,constra,variant) as r:marked_reduction) =
  if !sound_mode
  then f_next r
  else
    try
      Selfun.find_inductive_variable_to_remove (fun vl ->
        cancel_induction (fun hyp1 hist1 ->
          TermsEq.simplify_constraints_rule f_next f_next_again (hyp1,concl,hist1,constra,variant)
        ) vl hyp hist 0
      ) r
...
let rec limit_depth n t =
   if n = 0 then
      ...
      | FAC(f,margs) ->
          let margs' = List.map (fun (t,k) -> limit_depth (n-1) t, k) margs in
          Terms.apply_symbol_margs f margs'
...
let limit_depth_rule (r:marked_reduction) =
  if !sound_mode then r else
  let r =
    let max_hyp = !Param.max_hyp in
    if max_hyp < 0 then r else
    let (hyp, concl, hist,constra,variant) = r in
    ...
    (max_length max_hyp hyp, concl,
     ...
     limit_length_constraints max_hyp constra,variant)
  in
   let max_depth = ! Param.max_depth in
   if max_depth < 0 then r else
   let (hyp, concl, hist,constra,variant) = r in
   (List.map (fun (t,mk) -> limit_depth_fact max_depth t, mk) hyp,
    limit_depth_fact max_depth concl, hist,
    Terms.map_constraints (limit_depth max_depth) constra,variant)
...
let rec pos_in_list init f1 = function
    [] -> -1
  | ((f2,_)::l) -> if equal_facts f1 f2 then init else pos_in_list (init+1) f1 l

let rec pos_in_list_marked init (f1,mk1) = function
  | [] -> None
  | (f2,mk2)::q -> 
      if equal_facts f1 f2 && (mk1 = mk2 && mk2 = NoMarking)
      then Some (init,q)
      else
        match pos_in_list_marked (init+1) (f1,mk1) q with
        | None -> None
        | Some(res,q') -> Some(res,(f2,mk2)::q')

let rec decompose_hyp_rec accu (hypl:(fact * marking) list) =
  List.fold_right (fun ((fact1,mk1) as hyp1:(fact * marking)) (hypl,nl,histl) ->
    let default() =
      if mk1 = NoMarking
      then 
        let count = pos_in_list (nl+1) fact1 hypl in
        if count >= 0 then
          (hypl, nl-1, Removed(nl, count, histl))
        else
          (hyp1::hypl, nl-1, histl)
      else
        match pos_in_list_marked (nl+1) hyp1 hypl with
        | None -> (hyp1::hypl, nl-1, histl)
        | Some (count,hypl') -> (hyp1::hypl', nl-1, Removed(count,nl,histl))
    in
    match fact1 with
      | Pred(chann,_) when chann == Param.event_pred_block || chann == Param.inj_event_pred_block || chann == Param.event2_pred_block -> 
          if mk1 <> NoMarking
          then Parsing_helper.internal_error "Events should not be marked";
          default ()
      | Pred(chann,l0) ->
        let rec try_equiv_set = function
          ...
          | (hypeq, concleq, neq)::l ->
              try
                let hypeq' =
                  AC.Ac_unify.match_facts (fun () -> 
                    List.map (fun fact -> copy_fact3 fact,NoMarking) hypeq
                  ) concleq fact1
                in
                let hypeq_mk = List.map (fun fact -> fact,NoMarking) hypeq in 
                let hist_dec = Rule(neq, LblEquiv, hypeq_mk, concleq, true_constraints) in
...
let decompose_hyp_tuples ((hypl, concl, hist, constra,variant) as rule:marked_reduction) =
  if hist_is_exempt_from_data_simplify hist then
   ...
   in
   ((hypl', concl, histl',constra,variant):marked_reduction)
...
let elim_any_x_all ((hypl', concl, histl', constra,variant):marked_reduction) : marked_reduction =
  ...
  let hypl_vars = List.map (fun (fact,_) -> Terms.get_vars_fact_nolink fact) hypl' in
  ...
  let (hypl'', _, histl'') =
    ...
    *)
    fold_right_prev3 (fun prev_vars curr_vars next_vars ((fact1,_) as hyp1) (hypl, nl, histl) -> match fact1 with
      | Pred(chann, l) ->
          begin
            try
              let (n, ff) =
  ...
  in
  (hypl'', concl, histl'',constra,variant)

let elim_any_x_empty_elim_set ((hypl', concl, histl', constra,variant):marked_reduction) : marked_reduction =
  if
  ...
  then
    ...
    let (hypl'', _, histl'') =
      fold_right_prev3 (fun prev_vars _ next_vars ((fact1,_) as hyp1) (hypl, nl, histl) -> match fact1 with
        | Pred(chann, l) ->
            if elim_any_x_condition chann l concl_vars prev_vars next_vars constra_vars
            then (hypl, nl-1, (Any(nl, histl)))
            else (hyp1 :: hypl, nl-1, histl)
    ...
    in
    (hypl'', concl, histl'',constra,variant)
  else (hypl',concl,histl',constra,variant)
...
let rec match_fact_with_hyp nextf (fact1 as hyp1) hyps2 passed_hyp =
  match hyps2 with
  | [] -> raise Unify
  | ((fact2,mk2) as hyp2::fact_l) ->
      try
        AC.Ac_unify.match_facts (fun () ->
          nextf (passed_hyp @ fact_l)
        ) fact1 fact2
      with Unify ->
        match_fact_with_hyp nextf hyp1 fact_l (hyp2 :: passed_hyp)
...
let rec match_fact_with_hyp_ordered nextf (fact1 as hyp1) ord1 ord_hyp2 passed_hyp =
  match ord_hyp2 with
  | [] -> raise Unify
  | (((fact2,mk2),(ord2,_)) as ord_fact2) :: fact_l ->
      try
      ...
      with Unify ->
        match_fact_with_hyp_ordered nextf hyp1 ord1 fact_l (ord_fact2 :: passed_hyp)
...
let rec has_unbound_var = function
  ...
  | FAC(_,margs) -> List.exists (fun (t,_) -> has_unbound_var t) margs
...
let reorder_and_split hyp =
  ...
  List.iter (fun (f,mk) ->
    ...
    then unbound_hyp := ((f,mk),fact_size f) :: !unbound_hyp
    else bound_hyp := (f,mk) :: !bound_hyp
  ) hyp;
...
let elim_not (next_stage:marked_reduction -> unit) ((hyp', _, _,_,_) as r:marked_reduction) =
  if (List.exists (fun (h,_) -> List.exists (fun fact -> AC.Ac_unify.are_facts_matched fact (Terms.unblock_predicate_fact h)) !not_set) hyp') then
    ()
  else
    next_stage r

let elim_not_solving apply_not (next_stage:marked_reduction -> unit) ((hyp', _, _,_,_) as r:marked_reduction) =
  if apply_not && (List.exists (fun (h,_) -> List.exists (fun fact -> AC.Ac_unify.are_facts_matched fact (Terms.unblock_predicate_fact h)) !not_set) hyp') then
    ()
  else
    next_stage r
...
let elim_taut (next_stage:marked_reduction -> unit) ((hyp', concl, _,_,_) as r:marked_reduction) = 
  if List.exists (fun (fact,_) -> equal_facts concl fact) hyp' then
    ()
  else
    next_stage r
...
let copy_marked_fact ((f,mk) as hyp) = 
  let f' = copy_fact f in 
  if f == f' then hyp else (f',mk)

let decompose_concl_tuples (next_stage:marked_reduction -> unit) ((hyp', concl, hist', constra,variant) as r:marked_reduction) =
  if hist_is_exempt_from_data_simplify hist' then
    next_stage r
  else
    let put_clause first_fact hist =
      ...
      let r = 
        Terms.auto_cleanup_nocatch (fun () ->
          (Terms.mapq_list copy_marked_fact hyp', copy_fact first_fact, hist, copy_constra constra, variant) 
        )
      in
      next_stage r
    in
          ...
          let rec try_equiv_set = function
            ...
            | (hypeq, concleq, neq)::l ->
                try
                  let hypeq' =
                    AC.Ac_unify.match_facts (fun () -> 
                    List.map (fun fact -> copy_fact3 fact) hypeq
                  ) concleq concl
                  in
                  List.iteri (fun n concl' ->
                    let hist_dec = Rule(neq + n + 1, LblEquiv, [concleq,NoMarking], List.nth hypeq n, true_constraints) in
                    let hist'' = Resolution(hist, 0, hist_dec) in
                    rec_call concl' hist''
                  ) hypeq'
                with Unify ->
        ...
        Pred(p, [t1; Var v']) when p.p_prop land Param.pred_ELEM != 0 && v == v' ->
          begin match History.get_rule_hist (RElem(preds, p)) with
            | Rule(_, _, hyp, _, _) as hist_dec ->
                ...
                ((List.map (function
                    Pred(p',_),_ -> Pred(p', [t1]),NoMarking
                  ) hyp) @ l', hist'')
...
let transform_rule v ((hyp, concl, hist, constra, variant):marked_reduction) : marked_reduction =
...
let check_occurs_rule p0 v ((hyp, concl, hist, constra,_) : marked_reduction) =
  List.exists (fun (fact,_) -> check_occurs_fact p0 v fact) hyp ||
  (check_occurs_concl v concl) ||
  exists_constraints (occurs_var v) constra

let rec elem_simplif (next_stage:marked_reduction -> unit) repeat_next_stage ((hyp, concl, hist, constra,_) as r:marked_reduction) =
...
let rec is_marked_term = function
  ...
  | FAC(_,margs) -> List.for_all (fun (t,_) -> is_marked_term t) margs
...
let rec match_redundant_fact_with_kept_facts restwork ((fact1,mk1) as hyp1) = function
  | [] -> raise Unify
  | (fact2,mk2) :: q2 ->
      try
        if not (Marking.match_marking mk1 mk2) then raise Unify;
        AC.Ac_unify.match_redundant_hyp_facts restwork false fact1 fact2
      with Unify ->
        match_redundant_fact_with_kept_facts restwork hyp1 q2

let rec match_redundant_fact_with_hyp restwork ((fact1,mk1) as hyp1) passed_hyp = function
  | [] -> raise Unify
  | ((fact2,mk2) as hyp2):: q2 ->
      try
        if not (Marking.match_marking mk1 mk2) then raise Unify;
        AC.Ac_unify.match_redundant_hyp_facts (fun () ->
          (* At that point, we need to keep fact2 *)
          restwork hyp2 (List.rev_append passed_hyp q2)
        ) true fact1 fact2
      with Unify ->
        match_redundant_fact_with_hyp restwork hyp1 (hyp2::passed_hyp) q2
...
let rec match_redundant_hyp restwork kept_hyp = function
  ...
  | ((fact,_) as hyp) :: q ->
      ...
      then match_redundant_hyp restwork (hyp::kept_hyp) q
      else
        try
        ...
        with Unify ->
          ...
          try
            match_redundant_fact_with_hyp (fun hyp' passed_hyp ->
              match_redundant_hyp restwork (hyp'::kept_hyp) passed_hyp
            ) hyp [] q
          with Unify ->
            (* No match was found hence [fact] must be kept and we mark it. *)
            AC.Ac_unify.mark_redundant_hyp_variables_fact fact;
            match_redundant_hyp restwork (hyp::kept_hyp) q

let elim_redundant_hyp (next_stage:marked_reduction -> unit) repeat_next_stage ((hyp,concl,hist,constra,variant) as r:marked_reduction) =
  if (!Param.redundant_hyp_elim) &&
    ((not (!Param.redundant_hyp_elim_begin_only)) ||
     (List.exists (function
         Pred({p_info = TestUnifP _}, _),_ -> false
       | Pred(p,_),_ -> p.p_prop land Param.pred_BLOCKING != 0
      ) hyp))
  then
    try
      Terms.auto_cleanup (fun () ->
        ...
        List.iter (fun (fact,_) -> Terms.get_vars_fact vars fact) hyp;

        (* We mark the variables of the conclusion *)
        AC.Ac_unify.mark_redundant_hyp_variables_fact concl;
        ...
        (* The facts in [bound_hyp] do not have unbound variables, meaning that all
           variables have been marked when executing [AC.Ac_unify.mark_redundant_hyp_variables_fact concl].
          ...
          then
            begin
              ...
              try
                ...
                TermsEq.implies_constraints_keepvars (concl::List.rev_map (fun (fact,_) -> fact ) hyp) constra constra_inst;
                ...
                let rule' = copy_rule2 (hyp,concl,hist,constra_inst,variant) in
    ...
    with Unify -> next_stage r
  else next_stage r

(* 9.b For ordered clauses *)

let match_redundant_facts_ordered f_next apply_mark ((fact1,mk1), (ord1,_)) ((fact2,mk2), (ord2,_)) =
  if not (Marking.match_marking mk1 mk2) then raise Unify;
  AC.Ac_unify.match_redundant_hyp_facts (fun () ->
    if not (Terms.equal_facts fact1 fact2)
    then Database.implies_ordering_function ord1 ord2;
    f_next ()
  ) apply_mark fact1 fact2
  

let rec match_redundant_fact_with_kept_facts_ordered restwork hyp1 = function
  | [] -> raise Unify
  | hyp2 :: q2 ->
      try
        match_redundant_facts_ordered restwork false hyp1 hyp2
      with Unify ->
        match_redundant_fact_with_kept_facts_ordered restwork hyp1 q2

let rec match_redundant_fact_with_hyp_ordered restwork hyp1 passed_hyp = function
  | [] -> raise Unify
  | hyp2 :: q2 ->
      try
        match_redundant_facts_ordered (fun () ->
          (* At that point, we need to keep fact2 *)
          restwork hyp2 (List.rev_append passed_hyp q2)
        ) true hyp1 hyp2
      with Unify ->
        match_redundant_fact_with_hyp_ordered restwork hyp1 (hyp2::passed_hyp) q2

let rec match_redundant_hyp_ordered restwork kept_hyp = function
  | [] -> restwork ()
  | (((fact,_),_) as o_fact) :: q ->
...
let elim_redundant_hyp_ordered next_stage repeat_next_stage ahist_op ((hyp, concl, hist, constra, variant) as rule:marked_reduction) =
  match ahist_op with
            ...
            match_redundant_hyp_ordered (fun () ->
              ...
              then
                begin
                  ...
                  try
                    (* We check the implication of constraints. *)
                    TermsEq.implies_constraints_keepvars (concl:: List.map (fun (fact,_) -> fact ) hyp) constra constra_inst;
...
let verify_application_condition last_analysed_hist (n_idx_opt,n) (m_idx,m) matched_facts ((_,concl,_,_,_) as rule:marked_reduction) lemma_concl induction in_solving =
  if in_solving
  then
    if induction
    then
              ...
              let exists_unif =
                List.exists (fun (f_list,_,_) ->
                  List.exists (fun fact ->
                    AC.Ac_unify.are_facts_unifiable (to_block_non_inj_event fact) block_concl
                  ) f_list
                ) lemma_concl
              in
...
let match_and_apply_lemma next_stage_no_match next_stage_match last_analyzed_hist ((hyp,concl,hist,constra,variant) as rule:marked_reduction) induction in_verification lemma =
  ...
  let hyp_index = List.mapi (fun i (f,_) -> (i,f)) hyp in
  ...
  let instantiates_only_existential_vars vars =
    not (List.exists (fun v -> List.memq v !variables_rule) vars)
  in
  ...
  *)
  ...
  let apply_lemma_if_options_allow match_hyp matched_sub =
    (* Printf.printf "   Apply lemma with ";
    Display.Text.display_list (fun (i,f) -> print_int i; print_string "/"; Display.Text.display_fact f) " -- " match_hyp;
    print_string "\n"; *)

    let accu_rule = ref [] in

    let generate_new_rule only_inst_exist_vars (fact_lem:fact list) constra_lem eqs_lem =
      let (fact_lem1,constra_lem1,hyp1,concl1,constra1) =
        Terms.auto_cleanup (fun () ->
          Terms.mapq_list Terms.copy_fact2 fact_lem,
          Terms.copy_constra2 constra_lem,
          Terms.mapq_list Terms.copy_marked_fact2 hyp,
          Terms.copy_fact2 concl,
          Terms.copy_constra2 constra
        )
      in

      let hyp2 = hyp1 @ List.map (fun fact -> fact,NoMarking) fact_lem1
      ...
      TermsEq.simplify_constraints_rule (fun rule ->
        ...
        if app_op = LAOnlyWhenRemove then raise Not_found;
        ...
        if app_op = LAOnlyWhenRemove then raise Not_found;
        accu_rule := (only_inst_exist_vars,rule) :: !accu_rule
      ) (hyp2,concl1,hist2,constra2,variant)
    in
    ...
    then
      begin
        ...
        List.iter (fun (fact_lem, constra_lem,eqs_lem) ->
          try
            AC.Ac_unify.unify_for_lemma_terms (fun () ->
              begin if instantiates_only_existential_vars (!current_bound_vars)
              then
                begin
          ...
          try
            AC.Ac_unify.unify_for_lemma_terms (fun () ->
              ...
              then
                    ...
                    then
                        ...
                        match !last_analyzed_hist_ref with
                          | None ->
                              List.for_all (fun fact ->
                                ...
                                List.exists (fun (fact,_) -> Terms.equal_facts fact1 fact) hyp
                              ) fact_lem
                          | Some ana_hist ->
                              let ord_fun_lem = generate_ordering_fun_from_lemma ana_hist.a_order_data match_hyp in
                              List.for_all (fun fact ->
                                let fact1 = Terms.copy_fact4 fact in
                                List.exists2 (fun (fact_hyp,_) (ord_fun_hyp,_) ->
                      ...
                      ((* constraints implied *)
                       try
                         ...
                         TermsEq.implies_constraints_keepvars4 (concl::(List.map (fun (fact,_) -> fact) hyp)) constra constra_lem;
                         true
                       with Unify -> false)
                    else
                      Terms.auto_cleanup (fun () ->
                ...
                begin 
                  Printf.printf "instantiates only exist\n";
                  generate_new_rule false fact_lem constra_lem eqs_lem
                end
              end;
              raise Unify
            ) exists_vars eqs_lem
    ...
    (* Printf.printf "   Number of new clauses: %d\n" (List.length !accu_rule);
    List.iter (fun (b,r) -> 
      Printf.printf "   Produced new clauses (%b): " b;
  ...
  let rec match_one_subterm f_next pos_l t_sub t = match t with
    | Var _ | FunApp({ f_cat = Name _; _},_) ->
        AC.Ac_unify.match_terms (fun () -> f_next (List.rev pos_l)) [t_sub,t]
    | FunApp(_,args) ->
        begin try
          AC.Ac_unify.match_terms (fun () -> f_next (List.rev pos_l)) [t_sub,t]
        with Unify -> match_one_subterm_list f_next pos_l 1 t_sub args
        end
    | FAC(_,margs) ->
        try
          AC.Ac_unify.match_terms (fun () -> f_next (List.rev pos_l)) [t_sub,t]
        with Unify -> match_one_subterm_mterm_list f_next pos_l 1 t_sub margs
  ...
  and match_one_subterm_list f_next pos_l i t_sub = function
    ...
    | t::q ->
        ...
        with Unify -> match_one_subterm_list f_next pos_l (i+1) t_sub q

  and match_one_subterm_mterm_list f_next pos_l i t_sub = function
    | [] -> raise Unify
    | (t,_)::q ->
        try
          match_one_subterm f_next (i::pos_l) t_sub t
        with Unify -> match_one_subterm_mterm_list f_next pos_l (i+1) t_sub q
  ...
  *)
  let rec match_all (matched_hyp:(int*fact) list) fl_hyp = function
    | [] ->
        ...
        let constra_implied = 
          try
            (* Check that implies_constra_list does not link new variables *)
            TermsEq.implies_constraints_keepvars3 (concl::hyp_nomarking) constra constra_premise;
            true
          with Unify -> false
        in
        if not constra_implied then raise Unify;
        ...
        match_subterms (fun matched_sub ->
          ...
          then 
            try 
              apply_lemma_if_options_allow matched_hyp matched_sub
        ...
        try
          ...
          *)
          ...
          let match_fun =
            ...
            then
              if i_hyp < 0
              then if induction then AC.Ac_unify.match_facts else AC.Ac_unify.match_facts_phase_geq
              else if induction then AC.Ac_unify.match_facts_unblock else AC.Ac_unify.match_facts_unblock_phase_geq
            else
              if induction
              then AC.Ac_unify.match_facts_unblock
              else
                if i_hyp < 0
                then AC.Ac_unify.match_facts_phase_geq
                else AC.Ac_unify.match_facts_unblock_phase_geq
          in

          match_fun (fun () ->
            match_all ((i_hyp,fact_lem)::matched_hyp) (prev_hyp @ q_hyp) q_lem
          ) fact_lem (event_of_inj_event fact_hyp)
        with Unify ->
  ...
  with Unify -> next_stage_no_match !last_analyzed_hist_ref

let simplify_lemmas (next_stage_no_match: analysed_history option -> marked_reduction -> unit) (next_stage:bool -> analysed_history option -> marked_reduction -> unit) in_solving lemmas inductive_lemmas last_analyzed_hist rule =
  ...
  let rec match_rule only_inst_exists_vars_opt last_analyzed_hist (rule:marked_reduction) =
    assert (!current_bound_vars == []);
    match_and_apply_all only_inst_exists_vars_opt last_analyzed_hist rule (lemmas,inductive_lemmas)

  and match_and_apply_all only_inst_exists_vars_opt last_analyzed_hist ((hyp,concl,hist,constra,variant) as rule:marked_reduction) (lemmas,inductive_lemmas) =
...
let simplify_natural_number (next_stage:marked_reduction -> unit) ((_,concl,_,constra,_) as rule:marked_reduction) = match concl with
...
let rec mark_variables_follow = function
  ...
  | FAC(_,margs) -> List.iter (fun (t,_) -> mark_variables_follow t) margs
...
let rec inter_variables vars = function
  ...
  | FAC(_,margs) -> List.iter (fun (t,_) -> inter_variables vars t) margs
...
let rec vars_not_in vars = function
  ...
  | FAC(_,margs) -> List.iter (fun (t,_) -> vars_not_in vars t) margs
...
let is_forced_removal_rule_bad ((hypl,concl,_,_,_):marked_reduction) = match concl with
  | Pred(p,[]) when p == Param.bad_pred ->
      List.for_all (function
        | Pred({p_info = AttackerBin _ | AttackerGuess _; _},args),_ -> List.for_all (function Var _ -> true | _ -> false) args
        | fact,_ -> Terms.is_unselectable fact
      ) hypl
  | _ -> false

let remove_useless_events_for_lemma_bad ((hypl,concl,hist,constra,variant) as rule:marked_reduction) =
  let rec remove_facts n hist = function
    | [] -> ([],hist)
    | (fact,_) as hyp :: q_fact ->
        let (q_fact',hist') = remove_facts (n+1) hist q_fact in

        if is_fact_only_for_lemma_bad fact
        then (q_fact',HMaxHyp(n,hist'))
        else (hyp::q_fact',hist')
  in
  ...
  then rule (* No modification *)
    ...
    TermsEq.simple_simplify_constraints_rule
      (elim_any_x (hypl',concl,hist',constra,variant))

let remove_useless_events_for_lemma_main in_saturation lemmas ((hypl,concl,hist,constra,variant) as rule:marked_reduction) =
  let check_one_fact_vs_one_fact_lemma fact sel_fact_end_or_inj_event_list begin_ev_fact_list fact_lem fact_lem_list =
    ...
    Terms.auto_cleanup (fun () ->
      try
            ...
            begin
              try
                ...
                List.iter (fun fact_ev ->
                  if List.exists (fun fact_lem' -> AC.Ac_unify.are_facts_unifiable (Terms.unblock_predicate_fact fact_ev) fact_lem') fact_lem_list
                  then
                    Terms.auto_cleanup (fun () ->
        ...
        ) (Terms.unblock_predicate_fact fact) fact_lem
      with Unify -> true
    )
  in
    ...
    match lemma.l_premise with
      | [fact_lem] ->
          ...
          then 
            Terms.auto_cleanup (fun () ->
              try
                AC.Ac_unify.match_facts_phase_geq (fun () -> true) fact_lem (Terms.unblock_predicate_fact fact)
              with Unify -> false
            )
          else false
      | _ ->
  ...
  let rec check_facts_in_hypl n rest_hypl hypl hist = match hypl with
    | [] -> ([],hist)
    | (fact,_) as hyp :: q_fact ->
        let (q_fact',hist') = check_facts_in_hypl (n+1) (hyp::rest_hypl) q_fact hist in
        ...
        then
	     ...
	     non-injective and blocking (as it is in hypothesis of the clause)*)
          ...
          if List.for_all (check_one_fact_vs_lemma fact (List.map (fun (f,_) -> f) hypl')) lemmas
          then
...
let simplify_conclusion (next_f:marked_reduction -> unit) ((hyp,concl,hist,constra,variant) as r:marked_reduction) = match concl with
  | Pred({ p_info = AttackerBin _ | AttackerGuess _;_} as p, [t1;t2]) ->
      let replace left =
        ...
        next_f (hyp,Pred(Param.bad_pred,[]),hist',constra,variant)
      in
...
let is_standard_clause ((_,_,hist,_,_) as r:marked_reduction) =
  (match hist with
  | Rule(_,tag,_,_,_) ->
      begin
        match tag with
...
let used_for_redundant ((hyps,_,_,_,_) :marked_reduction) = 
  List.for_all (fun (f,_) -> is_unselectable f) hyps
...
let normal_rule_solving apply_not database lemmas inductive_lemmas ord_rule =
  ...
  let selfun_add ord_r = 
    let (hyp, _, hist, constra, variant) = ord_r.rule in
    let sel_index = Selfun.selection_fun_nostatechange (hyp, Param.dummy_fact, hist, constra, variant) in
    if sel_index == -1
    then
      match ord_r.order_data with
        | None -> sel_index, ord_r
        | Some order_data ->
            let (sel_index', order_data') = Selfun.selection_with_ignore_nounif hyp order_data in
            sel_index',{ ord_r with order_data = Some order_data'}
    else sel_index, ord_r
  in
  ...
  and normal_rule_add_rule ana_hist_op rule =
    ...
    Database.DBOrd.add_rule database (fun _ _ _ -> false) selfun_add (fun ord_r -> false) ord_rule
...
let rec replace_and_copy2_aux l1 l = match l1 with
  | [] -> List.map copy_marked_fact2 l
  | t::q -> (copy_marked_fact2 t)::(replace_and_copy2_aux q l)
...
let rec replace_and_copy2 l1 n = function
  ...
  | (a::l) ->
      ...
      else (copy_marked_fact2 a)::(replace_and_copy2 l1 (n-1) l)

let compos (next_stage:marked_reduction -> unit) ((hyp1, concl1, hist1,constra1,variant1) as cl_solved:marked_reduction) ((hyp2, concl2, hist2,constra2,variant2):marked_reduction) (sel_index, fact, mark) =
  (* compose with the selected fact *)
  assert (!current_bound_vars == []);

  (* Printf.printf "** Resolution:\n";
  Printf.printf "  Solved rule: ";
  Display.Text.display_rule_indep cl_solved;
  Printf.printf "  Unsolved rule: " ;
  Display.Text.display_rule_indep (hyp2, concl2, hist2,constra2,variant2);
  Printf.printf "  Fact (%d): " sel_index;
  Display.Text.display_marked_fact (fact,mark);
  Printf.printf "\n";
  flush_all ();

  let i = ref 0 in *)

  match mark, Marking.is_ac_constructor_clause cl_solved with
  | (Static f1 | Dynamic f1), Some f2 when f1 == f2 -> () 
  | _ ->
      let all_clauses = ref [] in

      begin try
        let external_factor = Marking.find_external_factor cl_solved fact in

        AC.Ac_unify.unify_facts (fun () ->
          let marked_hyp1 = match external_factor with
            | None -> hyp1
            | Some (f,t) -> Marking.apply_dynamic_marking (Terms.copy_term4 t) f (List.map Terms.copy_marked_fact4 hyp1)
          in

          let new_cl' = 
            Terms.auto_cleanup (fun () ->
              let hyp' = replace_and_copy2 marked_hyp1 sel_index hyp2 in
          ...
          all_clauses := new_cl' :: !all_clauses;
          (* incr i;
          Printf.printf "Nb unif = %d\n" !i;
          flush_all (); *)
          raise Terms.Unify
        ) concl1 fact
      with Unify -> ()
      ...
      List.iter (Display.Text.display_rule_indep) !all_clauses; *)
      
      List.iter next_stage !all_clauses

let compos_solving next_stage sat_rule (ord_rule,sel_index,fact,mark, ord_data_op) =
  compos (fun rule' ->
    ...
    next_stage ord_rule'
  ) sat_rule.sat_rule ord_rule.rule (sel_index,fact,mark)
...
let apply_rewrite_rule_match t1 t2 = match Terms.xor_fun.f_cat with
  | Eq eqlist ->
      let accu = ref [] in
      List.iter (fun (lhd,rhs) ->
        let (leq', req',_) = 
          Terms.auto_cleanup (fun () -> 
            copy_red (lhd,rhs,true_constraints)
          )
        in
        try 
          AC.Ac_unify.match_terms (fun () ->
            let t' = Terms.copy_term3 req' in
            begin try 
              TermsEq.simp_eq t';
              accu := t' :: !accu;
            with TermsEq.Reduces -> ();
...
let saturate_hypotheses_for_non_variant (((hypl,concl,hist,constr,variant),_) as annot_cl) =
  ...
  then annot_cl
    ...
    begin 
      ...
      let rec apply_rewrite_rules p1 phase1 t1 = function
        | [] -> []
        | (p2,phase2,t2) :: q ->
            let new_terms = apply_rewrite_rule_match t1 t2 in
            if new_terms = []
            then apply_rewrite_rules p1 phase1 t1 q
            else
              let new_p = if phase1 > phase2 then p1 else p2 in
              let new_facts = List.map (fun t -> Pred(new_p,[t]),NoMarking) new_terms in
              List.rev_append new_facts (apply_rewrite_rules p1 phase1 t1 q)
      in

      let rec explore_hyp prev_hyp = function
        | [] -> ()
        | (Pred({ p_info = Attacker(phase,_)} as p,[t]),_) :: q ->
            let added_facts = apply_rewrite_rules p phase t prev_hyp in
            let added_facts' = 
              List.filter (fun (fact,_) ->
                not (List.exists (fun (p,_,t) -> Terms.equal_facts fact (Pred(p,[t]))) prev_hyp)
              ) added_facts
            in
            accu_hyp := List.rev_append added_facts' !accu_hyp;
            explore_hyp ((p,phase,t)::prev_hyp) (List.rev_append added_facts' q)
        | _ :: q -> explore_hyp prev_hyp q
      in
      ...
      then annot_cl
        ...
        Database.SubClause.generate_subsumption_data (hypl @ !accu_hyp,concl,hist,constr,variant)
    end
...
let redundant is_solved ruleset queue ((initrule, _) as annot_initrule) =
  ...
  let ((_,initconcl,_,_,variant) as initrule',_) as annot_initrule' = 
    if is_solved 
    then annot_initrule
    else saturate_hypotheses_for_non_variant annot_initrule
  in

  (* if annot_initrule != annot_initrule'
  then 
    begin
      Printf.printf "  Found some application of variant. New rule: ";
      Display.Text.display_rule_indep initrule';
    end; *)
  ...
  let rec redundant_rec ((hyp, _, hist, constra,variant) as rule, _) first =
    ...
    if sel_index != -1 then
      ...
      let (sel_fact,mk) = List.nth hyp sel_index in

      if first || not (is_pred_to_prove_fact sel_fact)
      then

        let apply clause add_data = 
          ...
          then
            let check_fun ((_,concl,_,_,_) as r:marked_reduction) =
              ...
              if AC.Ac_unify.are_facts_matched concl initconcl then
                ...
                if Database.SubClause.implies_redundant annot_r' annot_initrule'
                then raise Redundant
                else redundant_rec annot_r' false
            in
            
            compos (simplify_rule_constra check_fun check_fun) clause rule (sel_index,sel_fact,mk)
        in

        let apply_set elt = 
          let (elt_clause,_,elt_add_data) = elt.Database.SetClause.annot_clause in
          apply elt_clause elt_add_data
        in

        let apply_queue (cl,_,_,add_data) = 
          apply cl add_data
        in

        Database.SetClause.iter_unifiable apply_set ruleset sel_fact;
...
let normal_rule database lemmas inductive_lemmas r =
  ...
  and normal_rule_add_rule r = 
    Database.DB.add_rule database (redundant false) (fun r -> Selfun.selection_fun_nostatechange r, r) used_for_redundant (remove_useless_events_for_lemma true !all_original_lemmas (limit_depth_rule r))
  in
  ...
  if is_standard_clause r then
    ...
    simplify_conclusion (Database.DB.add_rule database (redundant false) (fun r -> Selfun.selection_fun_nostatechange r, r) used_for_redundant) r
  else
    normal_rule true r
...
(* let redundant_solving sat_rules first_rule_set ((({ rule = (_,target_concl,_, _,_); _ } as target_rule), _) as annot_target_rule) =

  let rec redundant_rec ((({ rule = (hyp,concl,hist,constra,_) } as ord_rule), _) as annot_ord_rule) seen_list =
    if AC.Ac_unify.are_facts_matched concl target_concl 
    then
      let sel_index = Selfun.selection_fun (hyp, Param.dummy_fact, hist, constra, false) in
      if (sel_index != -1) && 
        let (sel_fact,_) = List.nth hyp sel_index in
        (not (is_pred_to_prove_fact sel_fact))
      then
...
let redundant_glob database is_solved annot_initrule =
  !Param.redundancy_test <> Param.NoRed && redundant is_solved database.Database.DB.base_solved database.queue annot_initrule

let redundant_glob_solving database sat_rules annot_target_rule = false
...
let complete_rules database lemmas inductive_lemmas =
  ...
  let rec search_fix_point () = match Database.QueueClause.get database.Database.DB.queue with
    | None ->
        Display.stop_dynamic_display ();
        Database.SetClause.to_list database.Database.DB.base_solved
    | Some ((((hyp,concl,_,_,_) as rule), feat_data, sub_data, add_data) as annot_rule) ->
        let sel_index = Selfun.selection_fun rule in
        let add_data = match add_data, sel_index with
          | Solved _, _ -> add_data
          | Unsolved(i,mk), j when i = j -> add_data
          (* | Solved _, i -> 
              print_string "Warning : a clause was solved and became unsolved\n";
              assert(false);
              Unsolved(i,snd (List.nth hyp i)) *)
          | Unsolved _, -1 -> 
              let new_add_data = Database.Solved (used_for_redundant rule) in
              Database.DB.subsumption_for_newly_solved database (rule,feat_data,sub_data,new_add_data);
              new_add_data
          | Unsolved _, i -> 
              (* print_string "Warning : a clause was unsolved and became solved\n"; *)
              Unsolved(i,snd (List.nth hyp i))
        in

        let ignore_redundant = match add_data with
          | Database.Solved _ -> false
        ...
        then 
          ...
          | Database.Solved _ -> 
              ...
              Database.SetClause.add database.Database.DB.base_solved annot_rule concl;
              Database.DB.display_dynamic_statistics database;
              Database.SetClause.iter_unifiable (fun elt -> match elt.annot_clause with
                | elt_clause,_,Database.Unsolved(elt_sel_index,elt_mark) ->
                    compos normal_rule rule elt_clause (elt_sel_index,elt.selected_fact,elt_mark)
                | _ -> Parsing_helper.internal_error "The rule should be unsolved."
                ) database.Database.DB.base_unsolved concl
          | Database.Unsolved(sel_index,mark) -> 
               (* Printf.printf "UnSolved : %d\n" sel_index;
              flush_all (); *)
              let (sel_fact,mk) = List.nth hyp sel_index in
              Database.SetClause.add database.Database.DB.base_unsolved annot_rule sel_fact;
              Database.DB.display_dynamic_statistics database;
              (* Printf.printf "UnSolved1 : %d\n" sel_index;
              flush_all (); *)
              Database.SetClause.iter_unifiable (fun ns_elt -> 
                let (ns_elt_clause,_,_) = ns_elt.annot_clause in
                compos normal_rule ns_elt_clause rule (sel_index,sel_fact,mark)
              ) database.Database.DB.base_solved sel_fact;
              (* Printf.printf "UnSolved2 : %d\n" sel_index;
              flush_all (); *)
          end;

        Database.DB.display_rule_during_completion database (rule,add_data);
        search_fix_point ()
  in
...
let complete_rules_solving ?(apply_not=false) sat_rules sat_rules_for_redundant_glob database lemmas inductive_lemmas =
  ...
  let rec search_fix_point () = match Database.QueueOrdClause.get database.Database.DBOrd.queue with
    | None ->
        ...
        Database.SetOrdClause.to_list database.Database.DBOrd.base_solved
    | Some ((ord_rule,feature_vector,sub_data,add_data) as annot_ord_rule) ->
        let selfun_add ord_r = 
          let (hyp, _, hist, constra, variant) = ord_r.rule in
          let sel_index = Selfun.selection_fun (hyp, Param.dummy_fact, hist, constra, variant) in
          if sel_index == -1
          then
            match ord_r.order_data with
              | None -> sel_index, ord_r
        ...
        in
        let sel_index,ord_rule = selfun_add ord_rule in
        let add_data = match add_data, sel_index with
          | Solved _, _ -> add_data
          | Unsolved(i,mk), j when i = j -> add_data
          (* | Solved _, i -> 
              print_string "Warning : a clause was solved and became unsolved\n";
              let (hyp,_,_,_,_) = ord_rule.rule in
              assert(false);
              Unsolved(i,snd (List.nth hyp i)) *)
          | Unsolved _, -1 -> 
              let new_add_data = Database.Solved false in
              Database.DBOrd.subsumption_for_newly_solved database (ord_rule,feature_vector,sub_data,new_add_data);
              new_add_data
          | Unsolved _, i -> 
              (* print_string "Warning : a clause was unsolved and became solved\n"; *)
              let (hyp,_,_,_,_) = ord_rule.rule in
        ...
        let (hyp,concl,_,_,_) = ord_rule.rule in
        begin match add_data with
        | Solved _ -> 
            if not (redundant_glob_solving database sat_rules_for_redundant_glob (ord_rule, sub_data))
            then
              (* The boolean here as no value here. *)
              Database.SetOrdClause.add database.Database.DBOrd.base_solved annot_ord_rule concl;
              Database.DBOrd.display_dynamic_statistics database
        | Unsolved(sel_index,mark) -> 
            let (sel_fact,_) = List.nth hyp sel_index in
            let sel_ord_data = match ord_rule.order_data with
              | None -> None
              | Some ord_data -> Some (List.nth ord_data sel_index)
            in
            Database.SetOrdClause.add database.Database.DBOrd.base_unsolved annot_ord_rule sel_fact;
            Database.DBOrd.display_dynamic_statistics database;
            Database.SetSatClause.iter_unifiable (fun sat_rule -> compos_solving normal_rule sat_rule (ord_rule,sel_index,sel_fact,mark,sel_ord_data)) sat_rules sel_fact
        end;

        Database.DBOrd.display_rule_during_completion database (ord_rule,add_data);
        search_fix_point ()
  in
  search_fix_point ()
...
let completion lemmas inductive_lemmas clauses =
  ...
  let complete_with_equations rulelist =
    ...
    let rulelisteq =
      let l = ref [] in
      Database.QueueClause.iter (fun (r,_,_,_) -> l := r::(!l)) database.Database.DB.queue;
      !l
    in
    if !Param.html_output then
      begin
  ...
  let sat_rules_for_redundant_glob =
    List.filter (fun sat_rule ->
      let (hyp1, _, _, _,_) = sat_rule.sat_rule in
      List.for_all (fun (fact,_) -> is_unselectable fact) hyp1
    ) sat_rules
  in
...
let internal_bad_derivable lemmas rulelist =
  ...
  List.filter (function
    | { sat_rule = (_, Pred(p, []), _, _,_); _} when p == Param.bad_pred -> true
    | _ -> false
  ) !saturated_rules
