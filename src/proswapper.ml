...
let rec has_destr = function
  ...
  | FunApp(f,l) ->
      (match f.f_cat with
	    | Red _ | Failure -> true
      |	Syntactic _ -> Parsing_helper.internal_error "syntactic symbols should never occur in processes"
      |	_ -> false) &&
      (List.exists has_destr l)
  | FAC _ -> Parsing_helper.internal_error "AC function symbols should not be used for equivalence yet."
