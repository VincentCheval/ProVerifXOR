...
| FAC(f,margs) -> 
    add_func f;
    List.iter (fun (t,_) -> fun_set_term t) margs
...
let fun_set rules =
   List.iter (fun (hyp, concl, _, constra,_) ->
     List.iter (fun (fact,_) -> fun_set_fact fact) hyp;
     fun_set_fact concl;
...
let rec output_term filename = function
  ...
  | FAC(f,margs) -> Parsing_helper.internal_error "Unexpected AC function symbol"
...
let output_body filename (hyp, concl, _, constra,_) =
  let hyp = List.map (fun (fact,_) -> fact) hyp in
   match hyp with
...
let output_rule filename ((hyp, concl,hist,constra,_)  as rule) =
  let hyp = List.map (fun (fact,_) -> fact) hyp in
...
let main filename rules queries =
  ...
  let filter_fun (_, Pred(p,l), _, _, _) =
  ...
  let rules =
    ...
    | ToGenerate(rl,gen) ->
        ...
        gen (fun r -> if filter_fun r then rules_ref := r :: !rules_ref);
        !rules_ref
  in
