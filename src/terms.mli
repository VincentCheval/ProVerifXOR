...
val is_sub_predicate : predicate -> predicate -> bool

val apply_symbol : funsymb -> term list -> term
val apply_symbol_margs : funsymb -> (term * int) list -> term
    
val string_of_funsymb : funsymb -> string
...
(* [val_gen tl] creates new variables of types [tl] and returns them in a
   list *)
val var_gen_mterm : typet list -> mterm list
...
(** [occurs_vars_all_fact bl fact] returns true when all variables occuring in [fact] are in [bl]. *)
val occurs_vars_all_fact : binder list -> fact -> bool
...
val copy_rule : marked_reduction -> marked_reduction
...
val auto_cleanup_nocatch : (unit -> 'a) -> 'a
val auto_cleanup_noreset : (unit -> 'a) -> 'a

(* Exception raised when unification fails *)
exception Unify
val occur_check : binder -> term -> unit
val ensure_not_fail : term -> unit
...
val copy_marked_fact4 : fact * marking -> fact * marking
...
val zero_xor_fun : funsymb
val zero_xor_term: term
val xor_fun : funsymb
