...
let rec add_n_terms n t q = match n with
  | 0 -> q
  | _ -> add_n_terms (n-1) t (t::q)
...
let apply_rewrite_rules_close_eq restwork f args = 
  let t' = Terms.apply_symbol f args in
  restwork t';
  match f.f_cat with
  | Eq eqlist ->
...
let rec close_term_eq restwork = function
  ...
  | FunApp(f,l) ->
      close_list_eq close_term_eq (apply_rewrite_rules_close_eq restwork f) l
  | FAC(f,margs)->
      close_mterm_list_eq (fun args ->
        close_AC_term_list_eq restwork f args
      ) margs

and close_mterm_list_eq restwork = function
  | [] -> restwork []
  | (t,n) :: q ->
      close_term_eq (fun t' ->
        close_mterm_list_eq (fun q' ->
          restwork (add_n_terms n t' q')
        ) q
      ) t

and close_AC_term_list_eq f_next f = function 
  | [] | [_] -> failwith "Impossible"
  | [t1;t2] -> apply_rewrite_rules_close_eq f_next f [t1;t2]
  | t::q ->
      close_AC_term_list_eq (fun t' ->
        apply_rewrite_rules_close_eq f_next f [t;t']
      ) f q
...
let close_marked_fact_eq restwork = function
    Pred(p,l),mk ->
      close_term_list_eq (fun l' -> restwork (Pred(p,l'),mk)) l

let close_fact_list_eq = close_list_eq close_fact_eq

let close_marked_fact_list_eq = close_list_eq close_marked_fact_eq

let close_rule_eq restwork ((hyp,concl,hist,constra,variant):marked_reduction) =
  close_marked_fact_list_eq (fun hyp' ->
    close_fact_eq (fun concl' ->
      close_constra_eq (fun constra' ->
        let r = 
          Terms.auto_cleanup (fun () ->
            let hyp'' = List.map copy_marked_fact2 hyp' in
...
let close_rule_hyp_eq restwork (hyp,concl,hist,constra,variant) =
  close_marked_fact_list_eq (fun hyp' ->
    close_constra_eq (fun constra' ->
      let r = 
        Terms.auto_cleanup ( fun () ->
          ...
          (hyp'', copy_fact2 concl, !histref, constra'',variant)
        )
      in
      restwork r;
    ) constra
...
let apply_rewrite_rules_close_destr_eq restwork accu_constra f args = 
...
let rec close_term_destr_eq accu_constra restwork = function
  ...
  | FunApp(f,l) ->
      close_list_destr_eq close_term_destr_eq accu_constra (fun accu_constra' l' ->
        apply_rewrite_rules_close_destr_eq restwork accu_constra' f l'
      ) l
  | FAC(f,margs) ->
      close_mterm_list_destr_eq (fun accu_constra_1 args ->
        close_AC_term_list_destr_eq restwork accu_constra_1 f args
      ) accu_constra margs

and close_mterm_list_destr_eq restwork accu_constra = function
  | [] -> restwork accu_constra []
  | (t,n) :: q ->
      close_term_destr_eq accu_constra (fun accu_constra_1 t' ->
        close_mterm_list_destr_eq (fun accu_constra_2 q' ->
          restwork accu_constra_2 (add_n_terms n t' q')
        ) accu_constra_1 q
      ) t

and close_AC_term_list_destr_eq f_next accu_constra f = function 
  | [] | [_] -> failwith "Impossible"
  | [t1;t2] -> apply_rewrite_rules_close_destr_eq f_next accu_constra f [t1;t2]
  | t::q ->
      close_AC_term_list_destr_eq (fun accu_constra_1 t' ->
        apply_rewrite_rules_close_destr_eq f_next accu_constra_1 f [t;t']
      ) accu_constra f q
        ...
        (fun accu_constra' l' ->
          Terms.auto_cleanup (fun () ->
            ...
            (* Check that the arguments of the predicate do not fail,
              ...
              List.iter Terms.ensure_not_fail l';
  ...
  close_list_destr_eq close_fact_destr_eq true_constraints (fun accu_constra hyp' ->
    close_fact_destr_eq accu_constra (fun accu_constra' concl' ->
      close_constra_destr_eq accu_constra' (fun accu_constra'' constra' ->
        let r = 
          Terms.auto_cleanup (fun () ->
            (List.map copy_fact2 hyp', copy_fact2 concl', copy_constra2 (wedge_constraints accu_constra'' constra'))
          )
        in
        restwork r
      ) constra
...
let rec normal_form = function
  ...
  | FAC _ -> Parsing_helper.internal_error "User defined equations should not contain AC function symbols."
...
| FAC _,_ | _, FAC _ -> Parsing_helper.internal_error "User defined equations should not contain AC function symbols."
...
let add_xor_convergent_system () =
  let x = new_var_def_term Param.bitstring_type in
  let xx = apply_symbol Terms.xor_fun [x;x] in
  let x_zero = apply_symbol Terms.xor_fun [x;zero_xor_term] in

  rewrite_system := (xx,zero_xor_term) :: (x_zero,x) :: !rewrite_system
...
let record_eqs_internal equations_list = 
  record_eqs_internal equations_list;
  Printf.printf "Using Xor = %b\n" !Param.using_xor;
  if !Param.using_xor then add_xor_convergent_system ()
...
let rec copy_remove_syntactic = function
  ...
  | FAC(f,margs) -> 
      let margs' = List.map (fun (t,k) -> copy_remove_syntactic t, k) margs in
      apply_symbol_margs (non_syntactic f) margs'
  | Var v ->
      match v.link with
      | NoLink ->
          let r = copy_var v in
...
let rec remove_syntactic_term = function
 ...
 | FAC(f,margs) -> 
    let margs' = List.map (fun (t,k) -> remove_syntactic_term t, k) margs in
    apply_symbol_margs (non_syntactic f) margs'
...
let remove_syntactic_marked_fact = function
  | Pred(chann, t),mk -> Pred(chann, List.map remove_syntactic_term t),mk


let rec remove_syntactic_constra c = Terms.map_constraints remove_syntactic_term c

let remove_syntactic_rule (hyp,concl,hist,constra,variant) =
  List.map remove_syntactic_marked_fact hyp,
...
let rec collect_unset_vars accu = function
  ...
  | FAC(_,margs) -> List.iter (fun (t,_) -> collect_unset_vars accu t) margs
  | Var v ->
      match v.link with
      | NoLink ->
...
let get_syntactic f = match f.f_cat with
  | Eq l when l <> [] ->
      begin try
...
let rec put_syntactic = function
  ...
  | FAC(f,margs) -> 
      let margs' = List.map (fun (t,k) -> put_syntactic t, k) margs in
      apply_symbol_margs (get_syntactic f) margs'
  | Var v ->
      match v.link with
      |	NoLink ->
...
let apply_rewrite_rules f_next f args = match f.f_cat with
  | Eq [] -> f_next (Terms.apply_symbol f args)
  | Eq eqlist ->
      begin 
        try 
          f_next (Terms.apply_symbol (get_syntactic f) args)
        with Unify ->
          let rec explore_eqlist = function
            | [] -> raise Unify
            | (leq,req)::q_eq -> 
                ...
                with Unify -> explore_eqlist q_eq
          in
          explore_eqlist eqlist
      end
  | _ -> f_next (Terms.apply_symbol f args)

(* Computing variant does not go through the links *)

let rec compute_variant f_next = function
  | Var { link = TLink t; _ } -> compute_variant f_next t
  | Var _ as t -> f_next t
  | FunApp(f,args) ->
      compute_variant_list (fun args' ->
        apply_rewrite_rules f_next f args'
      ) args
  | FAC(f,margs)-> 
      compute_variant_mterm_args (fun args ->
        compute_variant_AC_args f_next f args
      ) margs

and compute_variant_list f_next = function
  | [] -> f_next []
  | t::q ->
      compute_variant (fun t' ->
        compute_variant_list (fun q' ->
          f_next (t'::q')
        ) q
      ) t
      
and compute_variant_mterm_args f_next = function
  | [] -> f_next []
  | (t,n) :: q ->
      compute_variant (fun t' ->
        compute_variant_mterm_args (fun q' ->
          f_next (add_n_terms n t' q')
        ) q
      ) t

and compute_variant_AC_args f_next f = function 
  | [] | [_] -> failwith "Impossible"
  | [t1;t2] -> apply_rewrite_rules f_next f [t1;t2]
  | t::q ->
      compute_variant_AC_args (fun t' ->
        apply_rewrite_rules f_next f [t;t']
      ) f q

let rec unify_modulo_list_slow f_next l1 l2 = match l1, l2 with
  | [], [] -> f_next ()
  | [], _ | _, [] -> failwith "Error size list"
  | t1::q1, t2::q2 -> 
      compute_variant (fun t1' ->
        compute_variant (fun t2' ->
          AC.Ac_unify.unify_terms (fun () -> 
            unify_modulo_list_slow f_next q1 q2
          ) [t1',t2']
        ) t2
      ) t1
...
*)
let close_term_eq_synt restwork t = compute_variant restwork t
...
let unify_modulo_list restwork l1 l2 = unify_modulo_list_slow restwork l1 l2

let unify_modulo restwork t1 t2 = unify_modulo_list_slow restwork [t1] [t2]
...
let rec replace_genvar_with_var univ_vars t = match t with
  | Var v ->
      begin match v.link with
      | TLink t' -> replace_genvar_with_var univ_vars t'
      | NoLink -> t
      | _ -> internal_error "Unexpected link [replace_genvar_with_var]"
      end;
  | FunApp({ f_cat = General_var; _} as f,[]) ->
      begin try
        Var (List.assq f !univ_vars)
      with Not_found ->
        let v = new_var ~orig:false (if !Param.tulafale != 1 then "@gen" else "gen") (snd f.f_type) in
        univ_vars := (f, v) :: !univ_vars;
        Var v
      end
  | FunApp({ f_cat = General_mayfail_var; _} as f,[]) ->
      begin try
        Var (List.assq f !univ_vars)
      with Not_found ->
        let v = new_var ~orig:false ~may_fail:true (if !Param.tulafale != 1 then "@genmf" else "genmf") (snd f.f_type) in
        univ_vars := (f, v) :: (!univ_vars);
        Var v
      end
  | FunApp(f,args) -> 
      let args' =  mapq_list (replace_genvar_with_var univ_vars) args in
      if args == args' then t else FunApp(f, args')
  | FAC(f,margs) ->
      let margs' =  mapq_list (fun (t,k) -> replace_genvar_with_var univ_vars t, k) margs in
      if margs == margs' then t else apply_symbol_margs f margs'

let rec replace_genvar_with_var_and_var_with_constants univ_vars t = match t with
  | Var v ->
      begin match v.link with
      | TLink t' -> replace_genvar_with_var_and_var_with_constants univ_vars t'
      | NoLink -> 
          let cst = 
            FunApp({ f_name = Renamable v.vname;
                                      f_type = [], v.btype;
                                      f_cat = SpecVar v;
                                      f_AC = false;
  ...
  | FunApp(f,args) -> 
      let args' =  mapq_list (replace_genvar_with_var_and_var_with_constants univ_vars) args in
      if args == args' then t else FunApp(f, args')
  | FAC(f,margs) ->
      let margs' =  mapq_list (fun (t,k) -> replace_genvar_with_var_and_var_with_constants univ_vars t, k) margs in
      if margs == margs' then t else apply_symbol_margs f margs'

let implies_constraint diseq_1 diseq_2 =
  let diseq_2' = 
    let assoc_gen_2 = ref [] in
    Terms.auto_cleanup (fun () ->
      List.map (fun (t1,t2) -> 
        replace_genvar_with_var assoc_gen_2 t1,
        replace_genvar_with_var assoc_gen_2 t2
      ) diseq_2
    )
  in
  ...
  try 
    AC.Ac_unify.unify_terms (fun () ->
      let diseq_1' = 
        let assoc_gen_1 = ref [] in
        Terms.auto_cleanup (fun () ->
          List.map (fun (t1,t2) -> 
            replace_genvar_with_var_and_var_with_constants assoc_gen_1 t1,
            replace_genvar_with_var_and_var_with_constants assoc_gen_1 t2
          ) diseq_1
        )
      in
      if try AC.Ac_unify.unify_terms (fun () -> true) diseq_1' with Unify -> false
      then raise Unify
      else false
    ) diseq_2'
  with Unify -> true 
...
let implies_constraint sc_list1 sc_list2 = 
  let r1 = implies_constraint sc_list1 sc_list2 in
  let r2 = implies_constraint2 sc_list1 sc_list2 in
  if r1 <> r2
  then 
    begin 
      Printf.printf "-- Inconsistency (old = %b, new = %b) :\n" r1 r2;
      print_string "List 1 = ";
      ...
      print_string "\n";
      ignore(implies_constraint2 ~display:true sc_list1 sc_list2)
    end;
  r1 *)
...
let rec check_vars_occurs has_gen_var keep_vars accu_nat_vars nat_vars = function
  ...
  | FAC(_,margs) -> List.iter (fun (t,_) -> check_vars_occurs has_gen_var keep_vars accu_nat_vars nat_vars t) margs
...
let rec has_gen_var = function
  ...
  | FAC(_,margs) -> List.exists (fun (t,_) -> has_gen_var t) margs
...
let rec elim_var_in_is_not_nat accu_vars accu_keep_nat_vars keep_vars nat_vars = function
  ...
  | FAC(f,margs) ->
      let margs' = List.map (fun (t,k) -> elim_var_in_is_not_nat accu_vars accu_keep_nat_vars keep_vars nat_vars t, k) margs in
      apply_symbol_margs f margs'
...
let unify_disequation nextf accu constra =
  ...
  close_disequation_eq (fun () ->
    try
      let new_disequation = 
        Terms.auto_cleanup (fun () ->
          make_disequation_from_unify !keep_vars assoc_gen_with_var !var_list 
        )
      in
...
let simplify_neq_conj accu_keep_vars nat_vars neq_conj =
  ...
  then []
    ...
    begin 
      (* print_string "** Simplify_neq_conj:\n   Before: ";
      Display.Text.display_constraints (Terms.copy_constra4 {neq = neq_conj; is_nat = []; is_not_nat = []; geq = []}); *)
    
  let r = 
  Terms.auto_cleanup (fun () ->
    let accu = ref [] in
    List.iter (unify_disequation elim_universal_variable accu) neq_conj;
    (* print_string "\n   after unify_disequation: ";
    Display.Text.display_constraints (Terms.copy_constra4 {neq = !accu; is_nat = []; is_not_nat = []; geq = []}); *)
    let accu' = ref [] in
  ...
  in
  (* print_string "\n   Result: ";
  Display.Text.display_constraints (Terms.copy_constra4 {neq = r; is_nat = []; is_not_nat = []; geq = []});
  print_string "\n"; *)
  r
  end
...
let simplify_neq_conj2 neq_conj =
  ...
  then neq_conj
    ...
    Terms.auto_cleanup (fun () ->
      let accu = ref [] in
      List.iter (unify_disequation elim_universal_variable accu) neq_conj;
      List.iter (function [] -> raise Unify | _ -> ()) !accu;
      !accu
    )
...
let simple_simplify_constraints_rule (hyp,concl,hist,c,variant) =
  let vars = get_vars_facts (concl::(List.map fst hyp)) in
  simplify_constraints_keepvars
    (fun c2 -> (hyp,concl,hist,c2,variant))
    (fun c2 ->
      (* The variables in !Terms.current_bound_vars are the variables
         that are made equal by the inequalities. We added inequalities to
         represents these equalities. *)
...
let simplify_constraints_rule f_next f_next_inst ((hypl,concl,hist,constra,variant) as rule:marked_reduction)=
    ...
    try
      ...
      then
        simplify_constraints_keepvars (fun constra' ->
          f_next (hypl,concl,hist,constra',variant)
        ) (fun constra' ->
          let r' = copy_rule2 (hypl,concl,hist,constra',variant) in
          Terms.auto_cleanup (fun () -> f_next_inst r')
        ) (get_vars_facts (concl::(List.map (fun (fact,_) -> fact) hypl))) constra
      else
...
let rec copy_term5 modified = function
  ...
  | FAC(f,margs) -> 
      let margs' = List.map (fun (t,k) -> copy_term5 modified t, k) margs in
      apply_symbol_margs f margs'
  ...
  if not
      (List.for_all (fun neq_disj2 ->
        List.exists (fun neq_disj1 -> implies_constraint neq_disj1 neq_disj2) constraints1.neq
          ) constraints2.neq) then raise Unify;
                ...
                match index_of_term assoc t1 with
                  | None ->
                      ...
                      begin match distance.(i2).(0) with
                        | N k -> if k > n then raise Unify
                        | Infinity -> raise Unify
                      end
                  | Some i1 ->
                      try
    ...
    | (FunApp(f,l)) as t ->
        ...
        let rec find_red = function
          ...
          | ((leq,req)::redl) ->
              ...
              with Unify -> find_red redl
        in
        find_red !rewrite_system
    | FAC(f,margs) as t ->
          List.iter (fun (t,_) -> simp_eq t) margs;
          let rec find_red = function
          | [] -> ()
          | ((leq,req)::redl) ->
              try
                ...
                let r = AC.Ac_unify.are_terms_matched_non_strict leq t in
                if r then raise Reduces else find_red redl
              with Unify -> find_red redl
        in
        (* print_string "Find_red2\n"; *)
        find_red !rewrite_system
      ...
      | t::q ->
          ...
          then find_subterm f_next subterm_list q
            ...
            | FAC(_,margs) -> 
                ...
                ) subterm_list (List.map (fun (t,_) -> t) margs)
    in
...
let record_eqs horn_state =
  has_equations := (horn_state.h_equations != []) || !Param.using_xor;
...
let record_eqs_with_destr pi_state =
  has_equations := (pi_state.pi_equations != []) || !Param.using_xor;
  if hasEquations() then
    begin
      if !Param.html_output then
	begin
