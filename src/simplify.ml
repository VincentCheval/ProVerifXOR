...
let rec fst_snd_term next_f = function
  ...
  | FAC(f,margs) ->
      fst_snd_mterm_list (fun margs1 margs2 ->
        next_f (Terms.apply_symbol_margs f margs1) (Terms.apply_symbol_margs f margs2)
      ) margs

and fst_snd_mterm_list f_next = function
  | [] -> f_next [] []
  | (t,k)::q -> 
      fst_snd_term (fun t1 t2 ->
        fst_snd_mterm_list (fun q1 q2 ->
          f_next ((t1,k)::q1) ((t2,k)::q2)
        ) q
      ) t
...
let rec rename_term subst_var subst_names = function
  ...
  | FAC(f,margs) -> 
      let margs' = List.map (fun (t,k) -> rename_term subst_var subst_names t, k) margs in
...
let rec verify_term free_var = function
  ...
  | FAC(_,margs) -> List.iter (fun (t,_) -> verify_term free_var t) margs
