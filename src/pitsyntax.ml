...
let get_apply_symb env (s,ext) tl =
  ...
  try
    ...
    | (EFun r) as x ->
        if r == Terms.xor_fun || r == Terms.zero_xor_fun then Param.using_xor := true;
...
let non_compromised_session = Terms.apply_symbol Param.session1 []
...
let rec term_of_enriched_term et = match et.et_desc with
  ...
  | ET_Var v -> Var v
  | ET_FunApp(f,args) -> Terms.apply_symbol f (List.map term_of_enriched_term args)
  | ET_Test(econd,ethen,eelse) ->
      ...
      then term_of_enriched_term eelse
        ...
        Terms.apply_symbol gtest_symb [cond; tthen; telse]
  | ET_Let(epat,eexp,ethen,eelse) ->
      ...
      pattern_of_enriched_pattern (fun cond_pat ->
        ...
        then telse 
          ...
          let gtest_symb = Terms.gtest_fun ethen.et_type in
          Terms.apply_symbol gtest_symb [cond_pat; tthen; telse]
      ) exp epat 
...
and pattern_of_enriched_pattern f_next exp epat = match epat.ep_desc with
  ...
  | EP_PatTuple(f,pargs) ->
      ...
      let exp_args = List.map (fun fp -> Terms.apply_symbol fp [exp]) proj_funs in

      let first_arg = List.hd exp_args in
      let success_f = Terms.success_fun (Terms.get_term_type first_arg) in
      let cond_pat = Terms.apply_symbol success_f [first_arg] in
  ...
  | EP_PatEqual et ->
      ...
      let cond_pat = Terms.apply_symbol (Terms.equal_no_fail_fun et.et_type) [t;exp] in
      f_next cond_pat
...
let rec enriched_term_of_emargs f = function
  | [] | [_,1] -> internal_error "AC function symbols should have arity 2"
  | [et,2] ->
      { et_desc = ET_FunApp(f,[et;et]); et_simple = true; et_simple_no_let = true; et_simple_no_restr = true; et_type = snd f.f_type; et_ext = Parsing_helper.dummy_ext }
  | [et1,1;et2,1] ->
      { et_desc = ET_FunApp(f,[et1;et2]); et_simple = true; et_simple_no_let = true; et_simple_no_restr = true; et_type = snd f.f_type; et_ext = Parsing_helper.dummy_ext }
  | (_,0)::q -> enriched_term_of_emargs f q
  | (et,k)::q -> 
      let et_q = enriched_term_of_emargs f ((et,k-1)::q) in
      { et_desc = ET_FunApp(f,[et;et_q]); et_simple = true; et_simple_no_let = true; et_simple_no_restr = true; et_type = snd f.f_type; et_ext = Parsing_helper.dummy_ext }
...
let rec enriched_term_of_term = function
  ...
  | FAC(f,margs) ->
      let emargs = List.map (fun (t,k) -> enriched_term_of_term t, k) margs in
      enriched_term_of_emargs f emargs
      ...
      | (EFun f, result_type) -> 
		...
		if state.q_in_premise then
		  input_error ("function " ^ s ^ " is defined by \"reduc\". Such a function cannot be used in the premise.") ext
            ...
            (Terms.apply_symbol f l', result_type)
          end
        ...
        begin
          match r.f_cat with
            | Name { prev_inputs_meaning = sl } ->
                ...
                in
                (Terms.apply_symbol r p, snd r.f_type)
...
let check_subterm_pred state e evl =
  ...
  let rec check_choice = function
    ...
    | FAC(_,margs) -> List.iter (fun (t,_) -> check_choice t) margs
    | _ -> ()
  in
...
let rec check_event state (f,e) = match f with
  ...
  | PGFunApp(("event",e'),tl0,at_op) ->
      ...
      if !Param.key_compromise == 0 then
        QSEvent(None,[],None,at_op',Terms.apply_symbol (get_event_fun state.q_env f tyl') tl')
      else
        QSEvent(None,[],None,at_op',Terms.apply_symbol (get_event_fun state.q_env f (Param.sid_type :: tyl')) ((Terms.new_var_def_term Param.sid_type)::tl'))
  | PGFunApp(("inj-event",e'),tl0,at_op) ->
      ...
      let (tl', tyl') = List.split (List.map (check_query_term state) tl) in
      if !Param.key_compromise == 0 then
        QSEvent(Some (Param.fresh_injective_index ()),[],None,at_op',Terms.apply_symbol (get_event_fun state.q_env f tyl') tl')
      else
        QSEvent(Some (Param.fresh_injective_index ()),[],None,at_op',Terms.apply_symbol (get_event_fun state.q_env f (Param.sid_type :: tyl')) ((Terms.new_var_def_term Param.sid_type)::tl'))
...
let initialize_env () =
  ...
  Display.record_id "xor" dummy_ext;
  global_env := StringMap.add "xor" (EFun Terms.xor_fun) (!global_env);

  Display.record_id "zero" dummy_ext;
  global_env := StringMap.add "zero" (EFun Terms.zero_xor_fun) (!global_env);
...
let f_eq_tuple f s ext =
  if f == Terms.xor_fun
  then input_error "function xor should not be used in equations or destructors" ext;
...
let rec check_eq_term f_allowed fail_allowed_top fail_allowed_all env (term,ext) =
  match term with
        ...
        begin try
          match StringMap.find s env with
            ...
            | EFun f ->
                ...
                f_allowed f s ext;
                Terms.apply_symbol f [], snd f.f_type
            | EName r ->
                f_allowed r s ext;
                FunApp (r, []), snd r.f_type
            | ELetEq (t,ty) -> t,ty
    ...
    | (PFunApp ((f,ext), tlist)) ->
      ...
      f_allowed f' f ext;
      (Terms.apply_symbol f' tl', result_type)

  | (PTuple tlist) ->
      let (tl', tyl) = List.split (List.map (check_eq_term f_allowed false fail_allowed_all env) tlist) in
      (Terms.apply_symbol (Terms.get_tuple_fun tyl) tl', Param.bitstring_type)
          ...
          | (tylhs,tyrhs)::r ->
              ...
              let red_list' =
                begin
                  ...
                  let tuple = Terms.apply_symbol tuple_symb var_list in

                  assert (!Terms.current_bound_vars == []);

                  let side_cond_neq =
                    List.map (fun (arg_list,_,_) ->
                      [tuple,Terms.apply_symbol tuple_symb (List.map (Terms.generalize_vars_not_in []) arg_list)]
                    ) red_list in
  ...
  | ET_FunApp(f,list_arg) ->
      ...
      let proc_layer cond_eval proc_context =
        proc_layer_list cond_eval (fun l ->
          ...
          | _ -> proc_context (Terms.apply_symbol f l))
      in
      proc_layer
      ...
      let proc_layer cond_eval context =
        layer_tl Terms.true_term (fun tl' ->
          let rec collect_tests accu_tests = function
            ...
            | FAC(f,margs) -> 
                let margs' = List.map (fun (t,k) -> collect_tests accu_tests t,k) margs in
      ...
      let proc_layer' proc_context proc_fail =
        proc_layer Terms.true_term (fun l' ->
          ...
          try
            ...
            let tfinal = proc_context (Terms.apply_symbol f l'') in
            match tests with
            | [] -> tfinal
            | [v,t] ->
  ...
  let pi_state =
    { pi_process_query = process_query;
      ...
      pi_funs = StringMap.fold (fun _ v accu ->
               match v with
               | EFun f -> 
                  if !Param.using_xor || (f != Terms.zero_xor_fun && f != Terms.xor_fun) then f::accu else accu
               | _ -> accu) (!global_env) [];
