...
let homogeneous_format ((p,args):fact_format) : fact_format = (p,homogeneous_format_term_list args)

let ac_fact_format_of_fact_format ((p,format_list):fact_format) : ac_fact_format = 
  assert(!Terms.current_bound_vars = []);

  let vars = 
    Terms.auto_cleanup (fun () ->
      List.iter mark_format_term format_list;
      !Terms.current_bound_vars
    )
  in

  let rec term_of_format = function
    | FFunApp(f,args) -> 
        let args' = List.map term_of_format args in
        Terms.apply_symbol f args'
    | FVar v | FAny v -> Var v
  in
  (* Printf.printf "** ac_fact_format_of_fact_format : ";
  Display.Text.display_ac_fact_format (Pred(p,List.map term_of_format format_list),vars);
  print_string "\n"; *)
  
  (Pred(p,List.map term_of_format format_list),vars)
...
let initialize v_no_unif_set solver_kind =
  ...
  List.iter (fun (f,nv,opt) ->
    ...
    List.iter (function
      | Hypothesis -> no_unif_set := add_no_unif (ac_fact_format_of_fact_format f') n !no_unif_set
      | Conclusion -> no_unif_concl_set := add_no_unif (ac_fact_format_of_fact_format f') n !no_unif_concl_set
      | InductionVerif -> no_unif_induction := ac_fact_format_of_fact_format f' :: !no_unif_induction
      | InductionSat vl -> no_unif_induction_saturation := (ac_fact_format_of_fact_format f',vl) :: !no_unif_induction_saturation
    ) opt;

    if !Param.nounif_ignore_once = Param.NIO_All && n < 0 && not (List.exists (fun op -> op = InductionVerif) opt)
    then no_unif_induction := ac_fact_format_of_fact_format f' :: !no_unif_induction
  ) v_no_unif_set;
...
let has_same_format (fact:fact) ((fact_format,vars):ac_fact_format) =
  try 
    AC.Ac_unify.match_facts (fun () ->
      List.iter (fun v -> match v.link with
        | TLink (Var _) -> ()
        | TLink _ -> raise Unify
        | _ -> Parsing_helper.internal_error "Unexpected link (has same format)"
      ) vars;
      true
    ) fact_format fact
  with Unify -> false

let match_format f_next (fact:fact) ((fact_format,vars):ac_fact_format) =
  AC.Ac_unify.match_facts (fun () ->
    List.iter (fun v -> match v.link with
      | TLink (Var _) -> ()
      | TLink _ -> raise Unify
      | _ -> Parsing_helper.internal_error "Unexpected link (has same format 2)"
    ) vars;
    f_next ()
  ) fact_format fact
...
let rec has_same_format_induction nextf fact ((fact_format,vars),vl2) =
  let vl_op =
    try 
      match_format (fun () ->
        Some (List.map (fun v2 -> match v2.link with
          | TLink (Var v2') -> v2'
...
let find_inductive_variable_to_remove nextf ((hyp,_,_,constra,_):marked_reduction) =
  ...
  let rec find_inductive_variable_hyp nextf v1 format = function
    ...
    | (fact2,_)::q2 ->
        try
          has_same_format_induction nextf fact2 format
  ...
  let rec find_inductive_variable_hyp_list nextf = function
    ...
    | (fact1,_)::q1 ->
        try
          ...
          ) fact1 !no_unif_induction_saturation
...
let selection_with_ignore_nounif hyp order_data =
  ...
  let rec explore_hyp n passed_induc hyp induc_auth = match hyp,induc_auth with
    ...
    | (fact,_):: q_hyp, (ord,b) :: q_auth ->
        if b > 0 && List.exists (has_same_format fact) !no_unif_induction
        then
...
let implies_fact_format ((fformat1,vars1):ac_fact_format) ((fformat2,vars2):ac_fact_format) = 
  try 
    AC.Ac_unify.match_facts (fun () ->
      List.iter (fun v -> match v.link with
        | TLink (Var v2) -> 
            if not (List.memq v2 vars2) then raise Unify
        | TLink _ -> raise Unify
        | _ -> Parsing_helper.internal_error "Unexpected link (has same format)"
      ) vars1;
      true
...
(* let implies_fact_format ac_format1 ac_format2 = 
  let r = implies_fact_format ac_format1 ac_format2 in
  print_string "** implies_fact_format of\n   ";
  Display.Text.display_ac_fact_format ac_format1;
  print_string "\n   ";
  Display.Text.display_ac_fact_format ac_format2;
  Printf.printf "\n   result = %b\n" r;
  r *)
...
let rec occurs_test_loop seen_vars v t =
   ...
   | FAC(_,ml) -> List.exists (fun (t,_) -> occurs_test_loop seen_vars v t) ml


let are_facts_matched_and_checked_loop fact1 fact2 =
  let r =
  try
    let vars1 = Terms.get_vars_fact_nolink fact1 in
  ...
  then
    begin
      Printf.printf "are_facts_matched_and_checked_loop = true between fact1 = ";
      Display.Text.display_fact fact1;
      print_string " and fact2 = ";
...
let compute_match_format_fact_with_loop f_next fact1 fact2 =
  try
    let vars1 = Terms.get_vars_fact_nolink fact1 in
    ...
    let ac_fact_format =
      AC.Ac_unify.match_facts (fun () ->
        ...
        then
          ...
          let vl = 
            List.filter (fun v -> match v.link with
              | TLink (Var _) -> false 
              | _ -> true
            ) vars1
          in
    ...
    in
    (* begin
      Printf.printf "** Format computed with checked loop: \n";
      Printf.printf "   fact1 = ";
      Display.Text.display_fact fact1;
      Printf.printf "\n   fact2 = ";
      Display.Text.display_fact fact2;
      Printf.printf "\n   format = ";
      Display.Text.display_ac_fact_format ac_fact_format;
      print_string "\n"
    end; *)
    f_next ac_fact_format
  with Unify -> ()

let compute_match_format_fact_no_loop fact1 fact2 =
  try
    let vars1 = Terms.get_vars_fact_nolink fact1 in
    ...
    in
    (* begin
      Printf.printf "Format computed no checked loop: ";
      Display.Text.display_ac_fact_format ac_fact_format;
      print_string "\n"
    end; *)
    ac_fact_format
  with Unify -> internal_error "Should be matched."
...
let term_warning ((hyp, concl, _, _,_) as rule:marked_reduction) =
...
let selection_fun_nounifset ((hyp, concl, _, _,_) as rule:marked_reduction) =
  let rec sel (nold, wold) n = function
      [] ->
        if (nold >= 0) && (are_facts_matched_and_checked_loop (fst (List.nth hyp nold)) concl) && (!modify_nounif) then
          term_warning(rule);
        nold
    | (f,_)::l when is_unselectable f ->
        ...
        sel (nold, wold) (n+1) l
    | (fact,_) ::l ->
        let wnew = find_same_format fact (!no_unif_set) in
        if wnew <> 0 then
          if wnew > wold
          ...
          begin
            if (are_facts_matched_and_checked_loop fact concl) && (!modify_nounif) then term_warning(rule);
            n
          end
  in
  if is_unselectable concl then
    ...
    (* The conclusion can be selected if we don't find better in
       the hypothesis *)
    let w = find_same_format concl !no_unif_concl_set in
    if w <> 0
    then sel (-1, w) 0 hyp
    else sel (-1, -1) 0 hyp
...
let selection_fun_nounifset_maxsize ((hyp, concl, _, _,_) as rule:marked_reduction) =
  let rec sel (nold, wold) n = function
      [] ->
        if (nold >= 0) && (are_facts_matched_and_checked_loop (fst (List.nth hyp nold)) concl) && (!modify_nounif) then
          term_warning(rule);
    ...
    | (f,_)::l when is_unselectable f ->
        ...
        sel (nold, wold) (n+1) l
    | (fact,_) ::l ->
        let wtmp = find_same_format fact (!no_unif_set) in
        let wnew =
          if wtmp <> 0
          then wtmp
          else fact_size fact
...
let is_pure_ac_attacker = function
  | Pred({ p_info = (Attacker _ | AttackerBin _); _ }, args), NoMarking ->
...
let selection_fun_weight ?(redundant=None) ((hyp, concl, _, _, _) as rule:marked_reduction) =
  ...
  let rec sel (nold, wold) (nold_m,wold_m,hold_m) n = function
    | [] ->
        ...
        *)
        if nold = -1 && nold_m = -1 && !modify_nounif then (* conclusion selected *)
          begin
            List.iter (function (h,_) ->
              compute_match_format_fact_with_loop (fun format ->
                      ...
                      if !Param.verbose_term then
                        begin
                          ...
                          Display.auto_cleanup_display (fun () ->
                            Display.Text.display_ac_fact_format format;
        ...
        then 
          begin 
            let format = compute_match_format_fact_no_loop hold_m concl in
    ...
    | (fact,mk)::l ->
        let avoid_nounif = match redundant with 
          | None -> false
          | Some vars_concl -> Terms.occurs_vars_all_fact vars_concl fact
        in
        let wnew =
          let wtmp_0 = match mk with
            | Static _ -> 0
            | _ -> if avoid_nounif then 0 else find_same_format fact (!no_unif_set) 
          in
          let wtmp_0 = 
            if not avoid_nounif && is_pure_ac_attacker (fact,mk)
            then min wtmp_0 default_ac_no_unif_weight
            else wtmp_0
          in
          let wtmp_1 =
            if not avoid_nounif && wtmp_0 > match_concl_weight && are_facts_matched_and_checked_loop fact concl
            then match_concl_weight
            else wtmp_0
          in
          if wtmp_1 <> 0 then wtmp_1 else
          if !Param.select_fun == Param.TermMaxsize 
          then 
            let size = fact_size fact in
            match fact with
            | Pred({ p_info = Attacker _; _}, [FAC(_,_)]) -> 
                if mk = NoMarking then size else size + 50 
            | _ -> size + 100
          else 0
        in
        ...
        then sel (nold, wold) (nold_m, wold_m, hold_m) (n+1) l
          ...
          if not avoid_nounif && AC.Ac_unify.are_facts_matched fact concl
          then 
            if wnew > wold_m 
            then sel (nold, wold) (n, wnew, fact) (n+1) l
  ...
  let wconcl =
    if is_unselectable concl then
      ...
      | _ ->
          (* The conclusion can be selected if we don't find better in
             the hypothesis *)
          let wtmp_0 = find_same_format concl !no_unif_concl_set in
          let wtmp_0 = 
            if is_pure_ac_attacker (concl,NoMarking)
            then min wtmp_0 match_concl_weight
            else wtmp_0
          in
          let wtmp_1 =
            if wtmp_0 > match_concl_weight && List.exists (fun (h,_) -> are_facts_matched_and_checked_loop concl h) hyp
            then match_concl_weight
            else wtmp_0
          in
...
let rec false_copy = function
  ...
  | FAC(f,margs) -> 
      let margs' = List.map (fun (t,k) -> false_copy t, k) margs in
...
let selection_fun ?(redundant=None) ((hyp,concl,hist,constra,variant) as rule:marked_reduction) =
  let r =
   match !Param.select_fun with
     Param.NounifsetMaxsize -> selection_fun_nounifset_maxsize rule
   | Param.Term | Param.TermMaxsize -> selection_fun_weight ~redundant:redundant rule
      ...
      begin
        let rule2 = 
          Terms.auto_cleanup (fun () ->
            ...
            (List.map (fun (f,mk) -> copy_fact2 f,mk) hyp, copy_fact2 concl, hist, copy_constra2 constra,variant) 
        ) in
        match !Param.select_fun with
          Param.NounifsetMaxsize -> selection_fun_nounifset_maxsize rule2
        | Param.Term | Param.TermMaxsize -> selection_fun_weight ~redundant:redundant rule2
...
let selection_fun_redundant (hypl,concl,hist,constra,variant) =
  let vars = Terms.get_vars_fact_nolink concl in
  modify_nounif := false;
  let r = selection_fun ~redundant:(Some vars) (hypl,Param.dummy_fact,hist,constra,variant) in
  modify_nounif := true;
  r
...
let guess_no_unif rulequeue =
  ...
  if (!no_unif_set = []) || (!Param.select_fun == Param.Term)
    ...
    Database.QueueClause.iter (fun (r,_,_,_) -> 
  ignore (selection_fun_weight r)) rulequeue

let selection_fun rule = selection_fun rule